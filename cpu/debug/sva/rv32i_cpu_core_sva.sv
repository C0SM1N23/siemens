...
module rv32i_cpu_core_sva (
    ...
    ack_in_trap :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) cpu_irq_ack_i |-> cpu_in_trap_i)
    else $error("[core] ack without cpu_in_trap_i");

    // eoi is a one-cycle pulse that only follows an interrupt-returning MRET
    eoi_pulse :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) cpu_irq_eoi_i |=> !cpu_irq_eoi_i)
    else $error("[core] cpu_irq_eoi_i longer than one cycle");

    eoi_after_mret :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) cpu_irq_eoi_i |-> $past(
        mret_exec_i && s2_advance_i
    ))
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) wfi_ar_seen_q <= 1'b0;
        else if (!wfi_wait_i) wfi_ar_seen_q <= 1'b0;
        else if (ib_arvalid_i && ib_arready_i) wfi_ar_seen_q <= 1'b1;
    end
