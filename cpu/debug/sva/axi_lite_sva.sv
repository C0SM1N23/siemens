...
module axi_lite_sva #(
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) rd_out_q <= 1'b0;
        else if (ar_hs && !r_hs) rd_out_q <= 1'b1;
    ...
    rd_max_one :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) ar_hs |-> !rd_out_q || r_hs)
    ...
    ar_reset_quiet :
    assert property (@(posedge clk_i) !rst_n_i |-> !arvalid_i)
    ...
    generate
        if (HAS_WRITE) begin : g_write
            ...
            always @(posedge clk_i or negedge rst_n_i) begin
                if (~rst_n_i) aw_done_q <= 1'b0;
                else if (aw_hs) aw_done_q <= 1'b1;
                else if (b_hs) aw_done_q <= 1'b0;
            end

            always @(posedge clk_i or negedge rst_n_i) begin
                if (~rst_n_i) w_done_q <= 1'b0;
                else if (w_hs) w_done_q <= 1'b1;
                else if (b_hs) w_done_q <= 1'b0;
            end
        ...
        bvalid_i |-> aw_done_q && w_done_q)
            ...
            aw_max_one :
            assert property (@(posedge clk_i) disable iff (!rst_n_i) aw_hs |-> !aw_done_q || b_hs)
            else $error("[%0s] second AW issued while one write in flight", NAME);

            w_max_one :
            assert property (@(posedge clk_i) disable iff (!rst_n_i) w_hs |-> !w_done_q || b_hs)
            ...
            awvalid_reset_quiet :
            assert property (@(posedge clk_i) !rst_n_i |-> !awvalid_i && !wvalid_i)
            else $error("[%0s] AW/W VALID asserted during reset", NAME);

            cov_aw_backpressure :
            cover property (@(posedge clk_i) disable iff (!rst_n_i) awvalid_i && !awready_i);
            cov_w_backpressure :
            cover property (@(posedge clk_i) disable iff (!rst_n_i) wvalid_i && !wready_i);
            cov_b_wait :
            cover property (@(posedge clk_i) disable iff (!rst_n_i) bvalid_i && !bready_i);
            cov_b_slverr :
            cover property (@(posedge clk_i) disable iff (!rst_n_i) b_hs && bresp_i == 2'b10);
            cov_b_decerr :
            cover property (@(posedge clk_i) disable iff (!rst_n_i) b_hs && bresp_i == 2'b11);
