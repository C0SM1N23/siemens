...
module rv32i_cpu_func_cov (
    ...
    integer i, j;
    wire signed [31:0] cls_i = cls_idx(opcode);
    initial begin
    ...
    always @(posedge clk_i) begin
        if (rst_n_i && commit) begin
            if (cls_i >= 0) cls_cnt[cls_i] <= cls_cnt[cls_i] + 1;
        end
    end
    ...
    always @(posedge clk_i) begin
        if (rst_n_i && s2_advance_i) begin
            if (mispredict_i && pred_taken_i && ctl_taken_i) bp_tgt_wrong <= bp_tgt_wrong + 1;
        end
    end
    ...
    always @(posedge clk_i) begin
        if (rst_n_i) begin
            if (commit && ras_pop_i) begin
                ...
                end else ret_pred_ok <= ret_pred_ok + 1;
            end
        end
    end
    ...
    always @(posedge clk_i) begin
        if (rst_n_i) begin
            if (cpu_irq_i && lsu_active_i) ev_irq_in_stall <= ev_irq_in_stall + 1;
        end
    end
    ...
    always @(posedge clk_i) begin
        if (rst_n_i) begin
            if (cpu_irq_ack_i) begin
                if (cpu_irq_vec_i < 4'd8)
                    ack_cnt[cpu_irq_vec_i[2:0]] <= ack_cnt[cpu_irq_vec_i[2:0]] + 1;
            end
        end
    end
    ...
    always @(posedge clk_i) begin
        if (rst_n_i) begin
            if (ib_r_hs) begin
                if (ib_rresp_i[1]) begin
                end else ib_ok <= ib_ok + 1;
            end
            if (db_r_hs) begin
                case (db_rresp_i)
    ...
    function void rep(input string name, input integer cnt, input bit req);
        begin
            ...
            else if (req) gate_fail = gate_fail + 1;
