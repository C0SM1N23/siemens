...
module pic_sva #(
    ...
    irq_is_registered_offer :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) cpu_irq_i == exp_irq)
    else $error("[pic] cpu_irq_i != registered offer");

    vec_is_registered_offer :
    ...
    offer_is_unmasked :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) offer_val_i |-> cpu_mask_i[res_id_i])
    ...
    depth_moves_by_claim_eoi :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) depth_i == ($past(
        depth_i
    ) + {4'd0, $past(
        claim_push_i
    )} - {4'd0, $past(
    ...
    eoi_no_underflow :
    assert property (@(posedge clk_i) disable iff (!rst_n_i)
    (cpu_irq_eoi_i && !has_active_i) |=> (depth_i == $past(
        depth_i
    )))
