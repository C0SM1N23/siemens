...
module rv32i_tb_cpu_axi;
  ...
  W_CNT_TIED    = 288/4, W_CNT_CYCW   = 292/4, W_CNT_CYCRUN  = 296/4;
    ...
    always @(posedge clk or negedge rst_n) begin : update_ack_cnt
        integer ch;
        if (~rst_n) begin
            for (ch = 0; ch < 8; ch = ch + 1) ack_cnt[ch] <= 0;
        end else begin
            if (pic_claim && pic_claim_id < 4'd8)
                ack_cnt[pic_claim_id[2:0]] <= ack_cnt[pic_claim_id[2:0]] + 1;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (cpu_irq && ien_d[cpu_irq_vec] !== 1'b1) vec_bad <= 1;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (pic_claim && pic_claim_id == 4'd3) ack3_age <= 4'd1;
            else if (!cpu_in_trap) ack3_age <= 4'd0;
            else if (ack3_age != 4'd0 && ack3_age != 4'd15) ack3_age <= ack3_age + 4'd1;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (uut.wfi_wait) begin
                if (!wfi_prev) begin
                end else if (ib_mon_rd - wfi_rd_entry > 32'd1) wfi_noisy <= 1;
            end
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (uut.wfi_wait) begin
                if (!wfi_prev) wfi_rd_entry <= ib_mon_rd;
            end
        end
    end
    ...
    task sweep_channel;
        ...
        begin
            while (!(db_awvalid === 1'b1 && db_awready === 1'b1 &&
                 db_awaddr === (`DMEM_BASE + W_TRIGGER*4)))
            @(posedge clk);
            irq_src[ch] = 1'b1;
    ...
    initial begin
        ...
        t_ack2_first = $time;
        if (cpu_in_trap !== 1'b1) $display("FAIL: cpu_in_trap not set on irq ack");
        ...
        while (!(db_arvalid === 1'b1 && db_araddr === (`DMEM_BASE + W_MAGIC * 4))) @(posedge clk);
        irq_src[2] = 1'b1;
    ...
    initial begin
        ...
        if (t_ack2_first != 0 && t_ack3 > t_ack2_first) begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: ch2 served before ch3 (ack2 @%0t, ack3 @%0t)", t_ack2_first, t_ack3
                );
        end else begin
            $display("FAIL: PIC priority order (ack2 @%0t, ack3 @%0t)", t_ack2_first, t_ack3);
            errors = errors + 1;
        end
        ...
        if (wfi_seen && wfi_cycles >= 20 && !wfi_noisy) begin
            if ($test$plusargs("verbose"))
                $display("PASS: WFI slept %0d cycles, <=1 ibus read per sleep", wfi_cycles);
        end else begin
            $display("FAIL: WFI quietness (seen %0d, cycles %0d, noisy %0d)", wfi_seen, wfi_cycles,
                     wfi_noisy);
            errors = errors + 1;
        end
        ...
        begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: perf counters alive (mtimeD %0d, mispred %0d, dstall %0d, wfi %0d, fwait %0d)",
                    dmem_inst.mem[W_MTIME_DEL],
                    dmem_inst.mem[W_HPM3],
                    dmem_inst.mem[W_HPM5],
                    dmem_inst.mem[W_HPM7],
                    dmem_inst.mem[W_HPM4]
                );
        end else begin
            $display(
                "FAIL: a perf counter stayed 0 (mtimeD %0d, mispred %0d, dstall %0d, wfi %0d, fwait %0d)",
                dmem_inst.mem[W_MTIME_DEL], dmem_inst.mem[W_HPM3], dmem_inst.mem[W_HPM5],
                dmem_inst.mem[W_HPM7], dmem_inst.mem[W_HPM4]);
            errors = errors + 1;
        end
        if (t_rbeat2 != 0 && t_ack2 > t_rbeat2) begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: irq #3 held through the AXI stall (R @%0t, ack @%0t)", t_rbeat2, t_ack2
                );
        end else begin
        ...
        if (imem_inst.READ_LAT == 0 && imem_inst.STALL_PROB == 0) begin
            if (dmem_inst.mem[W_CPI] >= 32'd33 && dmem_inst.mem[W_CPI] <= 32'd44) begin
                if ($test$plusargs("verbose"))
        ...
        end else $display("SKIP: CPI check (imem latency/backpressure active)");
        ...
        begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: traffic seen (ibus rd %0d, dbus rd %0d wr %0d, pic rd %0d wr %0d, tmr rd %0d wr %0d)",
                    ib_mon_rd,
                    db_mon_rd,
                    db_mon_wr,
                    p_mon_rd,
        ...
        check(32'd0, dmem_inst.mem[W_CNT_HI0], "mcycleh/minstreth/mhpm3h/mhpm7h exist and read 0");
        check(32'hDEADBEEF, dmem_inst.mem[W_CNT_H3H], "mhpmcounter3h writable from M-mode");
        check(32'h000002C1, dmem_inst.mem[W_CNT_H6], "mhpmcounter6 low half writable from M-mode");
        check(32'd0, dmem_inst.mem[W_CNT_TIED],
              "HPM counters/selectors: fixed values, writes ignored");
        ...
        if (dmem_inst.mem[W_CNT_CYCW] < 32'd256) begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: mcycle write landed (readback = base + %0d)", dmem_inst.mem[W_CNT_CYCW]
                );
        end else begin
        ...
        begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: mcycle keeps counting after a write (+%0d -> +%0d)",
                    dmem_inst.mem[W_CNT_CYCW],
                    dmem_inst.mem[W_CNT_CYCRUN]
                );
        end else begin
            $display("FAIL: mcycle after write (+%0d -> +%0d)", dmem_inst.mem[W_CNT_CYCW],
                     dmem_inst.mem[W_CNT_CYCRUN]);
            errors = errors + 1;
        end
        ...
        if (imem_inst.READ_LAT == 0 && imem_inst.STALL_PROB == 0)
            check(32'd0, dmem_inst.mem[W_CNT_CYCW], "mcycle write lands exactly (latency-1 imem)");
        ...
        begin
            if ($test$plusargs("verbose"))
                $display(
                    "PASS: counters (mcycle %0d > minstret %0d)",
                    uut.csr_file_inst.mcycle_q,
                    uut.csr_file_inst.minstret_q
                );
        end else begin
            $display("FAIL: counters (mcycle %0d, minstret %0d)", uut.csr_file_inst.mcycle_q,
                     uut.csr_file_inst.minstret_q);
            errors = errors + 1;
        end
