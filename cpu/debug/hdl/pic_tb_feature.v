...
module pic_tb_feature;
    ...
    task wait_offer(input [3:0] ev);
        ...
        begin
            ...
            if (!(cpu_irq === 1'b1 && cpu_irq_vec === ev)) begin
                $display("FAIL: no offer for src %0d (cpu_irq=%b vec=%0d)", ev, cpu_irq,
                         cpu_irq_vec);
                errors = errors + 1;
            end
        end
    ...
    task no_offer_for(input [3:0] nev, input [511:0] msg);  // 64 chars, same as check
        begin  // cpu_irq must not currently present nev
            if (cpu_irq === 1'b1 && cpu_irq_vec === nev) begin
                ...
                if ($test$plusargs("verbose")) $display("PASS: %0s", msg);
            end
        end
    endtask
    ...
    initial begin
        ...
        axi_read(NEST_MAX_R, RESP_OKAY);
        check(32'd8, rd, "NEST_MAX default");
        axi_read(INT_ENABLE, RESP_OKAY);
        check(32'd0, rd, "INT_ENABLE default");
        axi_read(INT_STATUS, RESP_OKAY);
        check(32'd0, rd, "INT_STATUS default");
        check(32'd0, {31'b0, cpu_irq}, "cpu_irq idle after reset");

        // Empty-stack EOI must not underflow or create an active source.
        pulse_eoi;
        pulse_eoi;
        step(2);
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd0, rd, "empty EOI leaves nesting status clear");
        axi_read(ACTIVE_VEC, RESP_OKAY);
        check(32'd0, rd, "empty EOI leaves no active source");
        axi_read(INT_STATUS, RESP_OKAY);
        check(32'd0, rd, "empty EOI leaves interrupt status clear");
        ...
        axi_read(STA(3), RESP_OKAY);
        check(32'h0000_0001, rd & 32'hF, "src3 status pending");
        claim(4'd3);
        step(2);
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd1, rd & 32'h1F, "depth = 1 after claim");
        axi_read(ACTIVE_VEC, RESP_OKAY);
        check(32'h0000_0103, rd, "ACTIVE_VEC = valid|id3");
        axi_read(STA(3), RESP_OKAY);
        check(32'h2, rd & 32'h2, "src3 status active");
        ...
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd2, rd & 32'h1F, "depth = 2 (nested)");
        axi_read(ACTIVE_VEC, RESP_OKAY);
        check(32'h0000_0109, rd, "top of stack = src9");
        irq_src[9] = 1'b0;
        pulse_eoi;
        step(2);  // return to src8 context
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd1, rd & 32'h1F, "depth = 1 after inner eoi");
        axi_read(ACTIVE_VEC, RESP_OKAY);
        check(32'h0000_0108, rd, "restored context = src8");
        irq_src[8] = 1'b0;
        pulse_eoi;
        step(2);
        axi_read(NEST_STATUS, RESP_OKAY);
        ...
        axi_read(INT_STATUS, RESP_OKAY);
        check(32'h4, rd & 32'h4, "INT_STATUS.OVF set");
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'h1_0000, rd & 32'h1_0000, "NEST_STATUS.OVF set");
        axi_write(NEST_MAX_R, 32'd8, RESP_OKAY);  // relax the limit
        claim(4'd9);  // now it preempts
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd2, rd & 32'h1F, "depth = 2 once limit relaxed");
        irq_src[9] = 1'b0;
        pulse_eoi;
        step(2);
        irq_src[8] = 1'b0;
        ...
        step(2);
        axi_read(STA(12), RESP_OKAY);
        check(32'h8, rd & 32'h8, "src12 status SPUR");
        axi_read(SPURIOUS_LOG, RESP_OKAY);
        check(32'h1000, rd, "SPURIOUS_LOG bit12");
        axi_read(INT_STATUS, RESP_OKAY);
        check(32'h1, rd & 32'h1, "INT_STATUS.SPUR");
        pulse_eoi;
        step(2);  // balance the accounted claim
        axi_write(SPURIOUS_LOG, 32'h1000, RESP_OKAY);  // W1C
        axi_read(SPURIOUS_LOG, RESP_OKAY);
        check(32'h0, rd, "SPURIOUS_LOG cleared (W1C)");
        axi_write(INT_ENABLE, 32'h0000, RESP_OKAY);
        axi_write(INT_STATUS, 32'h7, RESP_OKAY);
        ...
        axi_read(STA(13), RESP_OKAY);
        check(32'h4, rd & 32'h4, "src13 status ESC");
        axi_read(INT_STATUS, RESP_OKAY);
        check(32'h2, rd & 32'h2, "INT_STATUS.ESC");
        claim(4'd13);
        irq_src[13] = 1'b0;
        irq_src[14] = 1'b0;
        pulse_eoi;
        step(2);
        axi_read(STA(13), RESP_OKAY);
        check(32'h0, rd & 32'hF, "src13 escalation cleared at idle");
        axi_write(INT_ENABLE, 32'h0000, RESP_OKAY);
        axi_write(INT_STATUS, 32'h7, RESP_OKAY);
        ...
        axi_read(SWT(15), RESP_OKAY);
        check(32'h0, rd, "sw channel still clear");
        axi_write(SWT(15), {SW_KEY, 16'h0001}, RESP_OKAY);  // keyed set
        wait_offer(4'd15);
        axi_read(SWT(15), RESP_OKAY);
        check(32'h1, rd, "sw channel set");
        claim(4'd15);  // consumed on claim
        step(2);
        axi_read(SWT(15), RESP_OKAY);
        check(32'h0, rd, "sw channel auto-cleared on claim");
        pulse_eoi;
        step(2);
        axi_write(INT_ENABLE, 32'h0000, RESP_OKAY);
        ...
        begin
            if ($test$plusargs("verbose")) $display("PASS: unmapped read -> SLVERR");
        end
        ...
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd2, rd & 32'h1F, "re-banded active src still preempted -> depth 2");
        begin
            if ($test$plusargs("verbose"))
        ...
        axi_read(NEST_STATUS, RESP_OKAY);
        check(32'd2, rd & 32'h1F, "escalated src preempted active src -> depth 2");
        begin
            if ($test$plusargs("verbose"))
        ...
        axi_read(STA(12), RESP_OKAY);
        check(32'h0, rd & 32'h30, "src12 effective band bumped to 0");
        axi_read(STA(12), RESP_OKAY);
        check(32'h4, rd & 32'h4, "src12 ESC flag set");
        claim(4'd12);
        irq_src[12] = 1'b0;
        pulse_eoi;
        step(2);
