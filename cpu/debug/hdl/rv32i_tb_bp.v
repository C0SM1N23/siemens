...
rv32i_branch_predictor #(
    ...
    initial begin
        ...
        if (dut.valid === {ENTRIES{1'b0}}) begin
            if ($test$plusargs("verbose")) $display("PASS:   valid array all zero");
        end else begin
            $display("FAIL:   valid array not all zero");
            errors = errors + 1;
        end
        if (dut.state === {ENTRIES{1'b0}}) begin
            if ($test$plusargs("verbose")) $display("PASS:   state array all zero");
        end else begin
            $display("FAIL:   state array not all zero");
            errors = errors + 1;
        end
        if (dut.isret === {ENTRIES{1'b0}}) begin
            if ($test$plusargs("verbose")) $display("PASS:   isret array all zero");
        end else begin
            $display("FAIL:   isret array not all zero");
            errors = errors + 1;
        end
        if (dut.tag === {(ENTRIES * TAG_W) {1'b0}}) begin
            if ($test$plusargs("verbose")) $display("PASS:   tag array all zero");
        end else begin
            $display("FAIL:   tag array not all zero");
            errors = errors + 1;
        end
        if (dut.target === {(ENTRIES * 32) {1'b0}}) begin
            if ($test$plusargs("verbose")) $display("PASS:   target array all zero");
        end else begin
            $display("FAIL:   target array not all zero");
            errors = errors + 1;
        end
        ...
        for (i = 0; i < ENTRIES; i = i + 1) begin
            ...
            if (pred_target !== (32'hC000_0000 + (i << 2))) begin
                $display("FAIL:   entry %0d target 0x%08h, expected 0x%08h", i, pred_target,
                         32'hC000_0000 + (i << 2));
                errors = errors + 1;
            end
        end
        ...
        look(pc_b);
        check(32'hBAD0_BAD0, pred_target, "  empty: the prediction falls back to the BTB target");
        ...
        for (i = 1; i <= RAS_DEPTH + 2; i = i + 1) begin
            upd(32'h0006_0180 + (i << 2), 1'b1, 32'h0007_0000, 1'b0, 1'b1, 1'b0, 32'hE000_0000 + i);
            if (i <= RAS_DEPTH) begin
                ...
                if (ras_count !== RAS_DEPTH[4:0]) begin
                    $display("FAIL:   after %0d pushes the depth is %0d, expected %0d", i,
                             ras_count, RAS_DEPTH);
                    errors = errors + 1;
                end
            end
        ...
        look(pc_b);
        check(32'hBAD0_BAD0, pred_target0, "  a return-tagged entry predicts from the BTB target");
        ...
        if (dut.target === {(ENTRIES * 32) {1'b0}}) begin
            if ($test$plusargs("verbose")) $display("PASS:   target array cleared");
        end else begin
            $display("FAIL:   target array not cleared");
            errors = errors + 1;
        end
