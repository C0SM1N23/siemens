...
module rv32i_tb_dual_core;
    ...
    initial begin
        ...
        for (i = 0; i < 1024; i = i + 1) dmem_inst.mem[i] = 32'b0;

        wait (rst_n === 1'b1);

        // run until both harts posted their results, or timeout
        tmo = 0;
        while (!(dmem_inst.mem[2] === 32'd111 && dmem_inst.mem[3] === 32'd222) && tmo < 20000) begin
            @(posedge clk);
            tmo = tmo + 1;
        end
        ...
        if (errors == 0) $display("== DUAL-CORE TEST PASSED ==");
        else $display("== %0d DUAL-CORE TESTS FAILED ==", errors);
        finish_test;
    end
