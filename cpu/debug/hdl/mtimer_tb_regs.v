...
mtimer dut (
    ...
    task chk_range(input [31:0] v, input [31:0] lo, input [31:0] span, input [511:0] name);
        begin
            if ((v >= lo) && (v <= lo + span)) begin
                if ($test$plusargs("verbose"))
                    $display(
                        "PASS: %0s = 0x%08h, inside [0x%08h .. 0x%08h]", name, v, lo, lo + span
                    );
            end else begin
    ...
    initial begin
        ...
        axil_read(MTIME_LO, RESP_OKAY);
        a0               = rd;
        first_read_cycle = axil_read_cycle;
        axil_read(MTIME_LO, RESP_OKAY);
        check(axil_read_cycle - first_read_cycle, rd - a0,
              "timer increments once per AR-to-AR clock");
    ...
    initial begin
        #500000;
        $fatal(1, "FAIL: tb_mtimer_regs timeout");
    end
