...
module axi_lite_monitor #(
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) begin
            // The err task updates this counter with blocking assignments.
            err_cnt_o = 0;
        end else begin
            ...
            if (armed) begin
                ...
                if (HAS_WRITE) begin
                    ...
                    if (b_hs && !(aw_pend && w_pend)) err("B beat before AW+W both accepted");
                end
            end
            ...
            if (armed) begin
                ...
                rvalid_i  === 1'bx || rready_i  === 1'bx)
                    err("X on AR/R valid/ready");
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) rd_out <= 0;
        else if (ar_hs) rd_out <= 1'b1;
        else if (r_hs) rd_out <= 1'b0;
    end
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) aw_pend <= 0;
        else if (HAS_WRITE && aw_hs) aw_pend <= 1'b1;
        else if (HAS_WRITE && b_hs) aw_pend <= 1'b0;
    end
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) w_pend <= 0;
        else if (HAS_WRITE && w_hs) w_pend <= 1'b1;
        else if (HAS_WRITE && b_hs) w_pend <= 1'b0;
    end
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) rd_cnt_o <= 0;
        else if (r_hs) rd_cnt_o <= rd_cnt_o + 1;
    end
    always @(posedge clk_i or negedge rst_n_i) begin
        if (!rst_n_i) wr_cnt_o <= 0;
        else if (HAS_WRITE && b_hs) wr_cnt_o <= wr_cnt_o + 1;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_arvalid <= arvalid_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_arready <= arready_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_araddr <= araddr_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_rvalid <= rvalid_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_rready <= rready_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_rdata <= rdata_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_rresp <= rresp_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_awvalid <= awvalid_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_awready <= awready_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_awaddr <= awaddr_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_wvalid <= wvalid_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_wready <= wready_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_wdata <= wdata_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_wstrb <= wstrb_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_bvalid <= bvalid_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_bready <= bready_i;
    end
    always @(posedge clk_i) begin
        if (rst_n_i) p_bresp <= bresp_i;
    end
