...
//
//     TIED OFF    mhpmcounter8..31, their upper halves, mhpmevent8..31.
...
rv32i_csr_file #(
    ...
    task csr_read(input [11:0] a, input exp_illegal, input [511:0] name);
        begin
            ...
            if (exp_illegal !== csr_illegal) begin
                $display("FAIL: %0s -> expected illegal=%b, got %b", name, exp_illegal,
                         csr_illegal);
                errors = errors + 1;
            end
            @(posedge clk) #1;
    ...
    initial begin
        ...
        if ($test$plusargs("verbose")) $display("   step 2: the identification CSRs");
        ...
        if (rd < 32'd64) begin
            if ($test$plusargs("verbose")) $display("PASS:   mcycle reset near 0, reads %0d", rd);
        end else begin
        ...
        if ($test$plusargs("verbose")) $display("   step 2: the four identification CSRs");
        ...
        if ($test$plusargs("verbose")) $display("   step 1: mhpmcounter8..31 (0xB08..0xB1F)");
        ...
        if ($test$plusargs("verbose")) $display("   step 2: mhpmcounter8..31h (0xB88..0xB9F)");
        for (i = 12'hB88; i <= 12'hB9F; i = i + 1) begin
            csr_read_chk(i[11:0], 32'h0000_0000, "  mhpmcounter8..31h reads 0");
            csr_write(i[11:0], 32'hFFFF_FFFF, 1'b0, "  write is accepted without trapping");
        end
        if ($test$plusargs("verbose")) $display("   step 3: mhpmevent8..31 (0x328..0x33F)");
        for (i = 12'h328; i <= 12'h33F; i = i + 1) begin
            csr_read_chk(i[11:0], 32'h0000_0000, "  mhpmevent8..31 reads 0");
            csr_write(i[11:0], 32'hFFFF_FFFF, 1'b0, "  write is accepted without trapping");
        end

        // Fixed selectors identify the five implemented events; writes are WARL.
        for (i = 12'h323; i <= 12'h327; i = i + 1) begin
            csr_read_chk(i[11:0], i - 32'h322, "fixed HPM event ID");
            csr_write(i[11:0], 32'h0, 1'b0, "fixed selector write accepted");
    ...
    initial begin
        #2000000;
        $fatal(1, "FAIL: tb_csr_ro timeout");
    end
