...
rv32i_alu_top dut (
    ...
    task eval(input [3:0] op, input [2:0] f3, input [6:0] f7, input [31:0] a, input [31:0] b,
    ...
    task eval_q(input [3:0] op, input [2:0] f3, input [6:0] f7, input [31:0] a, input [31:0] b,
                input [31:0] expected, input [511:0] name);
        begin
            aluop   = op;
            funct3  = f3;
            ...
            if (expected !== result) begin
                $display("FAIL: %0s -> expected 0x%08h, got 0x%08h", name, expected, result);
                errors = errors + 1;
            end
        end
    endtask
    ...
    initial begin
        ...
        funct7  = F7_0;
        opa     = 32'h0000_1000;
        opb_reg = 32'h0000_0020;
        opb_imm = 32'h0000_0300;
        ...
        for (i = 0; i < 8; i = i + 1) begin
            for (k = 0; k < 2; k = k + 1) begin
                ...
                if (expect_v !== result) begin
                    $display("FAIL:   I-type f3=%03b f7=%b -> expected 0x%08h, got 0x%08h", i[2:0],
                             funct7, expect_v, result);
                    errors = errors + 1;
                end
            end
        end
        ...
        eval(`ALUOP_FUNCT, 3'b000, F7_0, MINUS1, MINUS1, 32'hFFFF_FFFE, "  -1 + -1 = -2");
        eval(`ALUOP_FUNCT, 3'b000, F7_0, INTMIN, INTMIN, ZERO, "  INT_MIN + INT_MIN = 0");

        eval(`ALUOP_FUNCT, 3'b000, F7_1, ZERO, ONE, MINUS1, "  0 - 1 borrows to -1");
        eval(`ALUOP_FUNCT, 3'b000, F7_1, INTMIN, ONE, INTMAX, "  INT_MIN - 1 wraps to INT_MAX");
        eval(`ALUOP_FUNCT, 3'b000, F7_1, PAT_A, PAT_A, ZERO, "  x - x = 0");
        ...
        for (i = 0; i < 32; i = i + 1)
        eval_q(`ALUOP_FUNCT, 3'b001, F7_0, PAT_A, i, PAT_A << i, "SLL");
        begin
            if ($test$plusargs("verbose")) $display("PASS:   SLL correct for all 32 shift amounts");
        end

        for (i = 0; i < 32; i = i + 1)
        eval_q(`ALUOP_FUNCT, 3'b101, F7_0, PAT_A, i, PAT_A >> i, "SRL");
        begin
            if ($test$plusargs("verbose"))
        ...
        for (i = 0; i < 32; i = i + 1)
        eval_q(`ALUOP_FUNCT, 3'b101, F7_1, PAT_A, i, $signed(PAT_A) >>> i, "SRA");
        begin
            if ($test$plusargs("verbose"))
        ...
        eval(`ALUOP_FUNCT, 3'b101, F7_0, INTMIN, 32'd31, ONE, "  SRL of the same value by 31 is 1");
        ...
        eval(`ALUOP_FUNCT, 3'b011, F7_0, ONE, MINUS1, ONE, "  SLTU: 1 < 0xFFFFFFFF is true");
        eval(`ALUOP_FUNCT, 3'b010, F7_0, INTMIN, INTMAX, ONE, "  SLT:  INT_MIN < INT_MAX is true");
