...
//
//   This is separated from the feature bench (pic_tb_feature.v) on purpose. A reset
...
module pic_tb_reset;
    ...
    task check_full_reset_map(input [511:0] tag);
        begin
            ...
            for (i = 0; i < 16; i = i + 1) begin
                ...
                check(RST_CFG, rd, "  SRCx_CONFIG   reset value");
                axil_read(SWT(i), RESP_OKAY);
                check(RST_SWT, rd, "  SRCx_SW_TRIG  reset value");
                axil_read(STA(i), RESP_OKAY);
                check(RST_STA, rd, "  SRCx_STATUS   reset value");
            end
    ...
    initial begin
        ...
        axil_step(4);
        if ($test$plusargs("verbose")) $display("   step 2: sample the slave outputs mid-reset");
        ...
        if ($test$plusargs("verbose")) $display("   step 3: sample the CPU-side outputs mid-reset");
    ...
    initial begin
        #500000;
        $fatal(1, "FAIL: tb_pic_reset timeout");
    end
