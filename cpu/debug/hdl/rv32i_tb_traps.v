...
rv32i_cpu_top #(
    ...
    function [31:0] I_ADDI;
        input [4:0] rd;
        input [4:0] rs1;
        input [11:0] imm;
        I_ADDI = enc_i(imm, rs1, 3'b000, rd, OP_OPIMM);
    endfunction
    function [31:0] I_LUI;
        input [4:0] rd;
        input [19:0] imm20;
        I_LUI = enc_u(imm20, rd, OP_LUI);
    endfunction
    function [31:0] I_LW;
        input [4:0] rd;
        input [4:0] rs1;
        input [11:0] imm;
        I_LW = enc_i(imm, rs1, 3'b010, rd, OP_LOAD);
    endfunction
    function [31:0] I_LH;
        input [4:0] rd;
        input [4:0] rs1;
        input [11:0] imm;
        I_LH = enc_i(imm, rs1, 3'b001, rd, OP_LOAD);
    endfunction
    function [31:0] I_SW;
        input [4:0] rs2;
        input [4:0] rs1;
        input [11:0] imm;
        I_SW = enc_s(imm, rs2, rs1, 3'b010, OP_STORE);
    endfunction
    function [31:0] I_SH;
        input [4:0] rs2;
        input [4:0] rs1;
        input [11:0] imm;
        I_SH = enc_s(imm, rs2, rs1, 3'b001, OP_STORE);
    endfunction
    function [31:0] I_JAL;
        input [4:0] rd;
        input [20:0] off;
        I_JAL = enc_j(off, rd, OP_JAL);
    endfunction
    function [31:0] I_JALR;
        input [4:0] rd;
        input [4:0] rs1;
        input [11:0] imm;
        I_JALR = enc_i(imm, rs1, 3'b000, rd, OP_JALR);
    endfunction
    function [31:0] I_CSRRW;
        input [4:0] rd;
        input [11:0] csr;
        input [4:0] rs1;
        I_CSRRW = enc_i(csr, rs1, 3'b001, rd, OP_SYSTEM);
    endfunction
    function [31:0] I_CSRRSI;
        input [4:0] rd;
        input [11:0] csr;
        input [4:0] uimm;
        I_CSRRSI = enc_i(csr, uimm, 3'b110, rd, OP_SYSTEM);
    endfunction
    function [31:0] I_CSRRS;
        input [4:0] rd;
        input [11:0] csr;
        input [4:0] rs1;
    ...
    task prog_init;
        begin
            for (i = 0; i < IWORDS; i = i + 1) prog[i] = NOP;
    ...
    task expect_trap(input [31:0] exp_mcause, input [31:0] exp_mepc, input [31:0] exp_mtval,
                     input [31:0] exp_handler);
        integer timeout;
        begin
            timeout = 0;
    ...
    initial begin
        ...
        if ($test$plusargs("verbose")) $display("   step 2: at 0x08, execute  jal x0, +6");
        if ($test$plusargs("verbose"))
            $display("   step 3: the branch target is 0x08 + 6 = 0x0E, and bit 1 is set");
        if ($test$plusargs("verbose"))
            $display("   step 4: expect mcause = 0, mepc = 0x08 (the jump itself),");
        if ($test$plusargs("verbose")) $display("           mtval = 0x0E (the illegal target)");
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 2: at 0x08, jump to 0x%08h, outside the instruction memory", BAD_FETCH
            );
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 2: place the word 0x%08h at 0x08 - opcode 1111111 is undefined", BAD_IW
            );
        if ($test$plusargs("verbose")) $display("   step 3: expect mcause = 2, mepc = 0x08,");
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display("   step 1: mtvec <- 0x%08h (direct mode)", HANDLER);
        if ($test$plusargs("verbose")) $display("   step 2: at 0x08, execute EBREAK");
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display("   step 1: mtvec <- 0x%08h (direct mode)", HANDLER);
        if ($test$plusargs("verbose")) $display("   step 2: at 0x08, execute ECALL");
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 2: at 0x08,  lui x1, 0x%05h   -> x1 = 0x%08h, a VALID address",
                DMEM_BASE[31:12],
                DMEM_BASE
            );
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 1: at 0x08,  lui x1, 0x%05h   -> x1 = 0x%08h", DMEM_BASE[31:12], DMEM_BASE
            );
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 1: at 0x08,  lui x1, 0x%05h   -> x1 = 0x%08h, UNMAPPED",
                BAD_DATA[31:12],
                BAD_DATA
            );
        if ($test$plusargs("verbose"))
            $display("   step 2: at 0x0C,  lw x5, 0(x1)     -> aligned, so alignment cannot fault");
        if ($test$plusargs("verbose")) $display("   step 3: the data memory answers DECERR");
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose")) $display("           cause 4 must win over cause 5");
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose")) $display("           external cause 16 (PIC vector 0)");
        run_irq_case(4'd0);
        if ($test$plusargs("verbose"))
        ...
        cpu_irq_vec = 4'd5;
        cpu_irq     = 1'b1;
        if ($test$plusargs("verbose")) $display("   step 3: hold it for 300 cycles");
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 1: mtvec <- 0x%08h, i.e. BASE = 0x%08h with MODE = 1",
                HANDLER | 32'd1,
                HANDLER
            );
        if ($test$plusargs("verbose"))
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 3: the handler entry must be 0x%08h + 4*19 = 0x%08h",
                HANDLER,
                HANDLER + 32'd76
            );
        run_irq_case(4'd3);
        ...
        if ($test$plusargs("verbose"))
            $display(
                "   step 1: vectored mtvec, interrupt vector 0 enters at 0x%08h", HANDLER + 32'd64
            );
        if ($test$plusargs("verbose"))
        ...
        cpu_irq = 1'b0;
        while (!marker_seen) @(posedge clk);
        check(32'd1, eoi_at_marker[31:0], "  the inner handler's MRET sent exactly one EOI");
        repeat (40) @(posedge clk);
        ...
        begin
            ...
            if (cpu_in_trap !== 1'b1) begin
                ...
                begin
                    if ($test$plusargs("verbose"))
                        $display(
                            "PASS:   mepc = 0x%08h, an instruction boundary in the program body",
                            the_mepc
                        );
                end else begin
