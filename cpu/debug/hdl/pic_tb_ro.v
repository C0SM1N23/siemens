...
module pic_tb_ro;
    ...
    task check_read_only(input [31:0] a, input [31:0] pattern, input [31:0] cmp_mask,
        ...
        begin
            ...
            if ((before_val & cmp_mask) == 32'd0) begin
                $display("FAIL: %0s - precondition, the compared bits are 0 so the test is vacuous",
                         name);
                errors = errors + 1;
            end
    ...
    task check_ro_counter(input [31:0] a, input [511:0] name);
        ...
        begin
            ...
            if (t1 == 16'hFFFF) begin
                ...
                if ($test$plusargs("verbose"))
                    $display(
                        "PASS: %0s kept counting across the rejected write (0x%04h -> 0x%04h)",
                        name,
                        t0,
                        t1
                    );
    ...
    initial begin
        ...
        for (i = 0; i < 16; i = i + 1) check_ro_counter(STA(i), "SRCx_STATUS.DDL_TIMER");
        ...
        if ($test$plusargs("verbose")) $display("   step 2: BAND_CONFIG - reserved [31:8]");
        ...
        if ($test$plusargs("verbose")) $display("   step 3: NEST_MAX - reserved [31:5]");
        ...
        if ($test$plusargs("verbose")) $display("   step 5: INT_ENABLE - reserved [31:16]");
        check_reserved(INT_ENABLE, 32'hFFFF_0000, "  INT_ENABLE reserved [31:16] read 0");
        axil_write(INT_ENABLE, 32'h0000_0000, RESP_OKAY);

        if ($test$plusargs("verbose")) $display("   step 6: SPURIOUS_LOG - reserved [31:16]");
        axil_read(SPURIOUS_LOG, RESP_OKAY);
        check(32'd0, rd & 32'hFFFF_0000, "  SPURIOUS_LOG reserved [31:16] read 0");

        if ($test$plusargs("verbose")) $display("   step 7: INT_STATUS - reserved [31:3]");
        ...
        axil_step(2);
        if ($test$plusargs("verbose")) $display("   step 2: read the register back");
        ...
        before_val = rd;
        if ($test$plusargs("verbose")) $display("   step 1: SRC8_STATUS holds 0x%08h", before_val);
        if ($test$plusargs("verbose"))
    ...
    initial begin
        #500000;
        $fatal(1, "FAIL: tb_pic_ro timeout");
    end
