...
def render_document(doc,api):
    ...
    def emit(title,body,level='subsection',parents=(),wide=False,brk=True):
    ...
    emit('Validity of the document',
         ...
         +para('Revision 2.1 of 21 September 2026 describes the delivered state of that RTL.'),brk=False)
    ...
    emit('Terms and abbreviations',T(['Term','Meaning'],sorted(terms),[.23,.77]),brk=False)
    emit('Relationship with other documents',
         ...
         +T(['Reference','Version / role'],[
        ('RV32I 3-Stage Pipeline CPU.pdf' if cpu else 'Programmable Interrupt Controller with Advanced Scheduling','Supplied project draft; functional requirements and implementation decisions.'),
        ('Spec table of contents and templates.pdf','Supplied document structure and verification-plan template.'),
        (f'{"Verification" if design else "Design"} Specification — {kind}','Companion specification, revision 2.1, 21 September 2026.'),
        ('RISC-V Unprivileged ISA / Privileged Architecture' if cpu else 'ARM AMBA AXI protocol','v20191213 / v20211203, machine mode' if cpu else 'IHI0022E, AXI4-Lite.'),
        ('ARM IHI0022E / IEEE 1364-2005' if cpu else 'IEEE 1364-2005','AXI4-Lite / Verilog RTL.' if cpu else 'Verilog RTL.')],[.50,.50]))
    ...
    if design:
        take('Scope and original requirements','Requirements and integration',parents=(('section','Specifications and system integration'),),
             lead='The first table names the brief and the RTL that this section compares. The second sets each requirement of the brief against the contract implemented for it, including the points where the two differ.')
        if not cpu:
            take('Module configuration and integration constraints','Configuration and integration limits',
                 lead='The module has no Verilog parameters; every configurable quantity is either fixed in the RTL or programmed through a register. The tables separate the two, then state what an integrator has to guarantee outside the block.')
        ...
        take('Architecture',level='section')
        if cpu:
        ...
        if cpu:
            ...
            emit('Hardware interface',para('The two memory ports and PIC link are independent interfaces on the same clock domain. Capacity parameters do not change their external bus widths.')+body+ports,parents=(('section','Interfaces'),))
            ...
            take('Hardware interface',parents=(('section','Interfaces'),),
                 lead='All ports share one clock domain and one asynchronous reset. The table gives the direction, width and meaning of every port: the 16 source inputs, the CPU handshake that carries the offer back and the claim and EOI forward, and the AXI4-Lite register window.')
            ...
            for title in list(pages):
                if title.startswith('Register fields:'):take(title,level='subsubsection')
        ...
        take('Reuse and integration by another developer','Reuse and reusability',level='section',
             lead='This block reuses no external component. The table is what another developer needs in order to use it: which files to compile, what to connect, what to tie off and which checks to run after integration.')
        ...
        for i,title in enumerate(modules):
            ...
            details={
                ...
                'rv32i_branch_predictor':'The lookup is combinational and an update is committed on posedge. The prediction and update policies, including aliases, cold entries and trap preservation, are specified under Branch prediction.',
            ...
            take(title,title.split(': ',1)[1],parents=(('section','Design of submodules'),) if i==0 else ())
        ...
        take('Verification objective and evidence','Overview and acceptance',parents=(('section','Verification methodology'),),
        ...
        emit('Implementation strategy and reference model',para('Directed tests exercise individual features and simultaneous events. Program/reference tests check end-to-end results. Protocol checks observe accepted transfers; state checks sample after nonblocking updates.')+para('The ISA reference generates an instruction-by-instruction expected trace and memory state. The CPU runs the same sequence with nominal and delayed responders.' if cpu else 'The independent priority model enumerates all 24 band-urgency permutations with source, enable and CPU masks. Expected winner and pending set are compared with the RTL; directed tests add temporal behaviors such as nesting and deadline escalation.')+para('The verification plan records an ID, scenario, expected result, status and test/checker. Its detailed cases appear in the annex; the environment below identifies the components that execute and check them.'))
        ...
        take('Assertions, monitors and bind','Monitors and checkers',
             lead='Checking is split between bus protocol and functional result, so that a failure points at one of the two. The first table lists each checker and what it observes; the second shows how each one is kept from passing without having seen traffic.')
        ...
        titles=['CPU integration','ISA reference','ALU','Predictor / RAS','Exceptions / traps','CSR access','Counters','Two harts','Asynchronous reset'] if cpu else ['Feature sequences','Priority reference','Simultaneous events','Asynchronous reset','Read-only registers','Status transitions','CPU integration','SoC integration']
        emit('Verification plan',para('PASS indicates the recorded regression completed with its expected results and zero checker errors. Each row identifies the test that exercises the listed behavior.')+T(['ID','Title','Expected result / scope','Status','Test / checker'],[(row[0],title,row[2],'PASS',row[1]) for title,row in zip(titles,cases)],[.08,.16,.37,.10,.29]),parents=(('section','Annex'),))
