...
def table(head,rows,widths=None,small=True):
    ...
    for row in rows:result+=' & '.join(str(x) for x in row)+r' \\ \addlinespace[3pt]'+'\n'
...
class Doc:
    def __init__(self,title,subject,filename,kind):
        ...
        self.pages=[]
        self.s=[r'\documentclass[11pt]{article}',r'\newcommand{\doctitle}{'+esc(title)+'}',r'\newcommand{\docsubject}{'+esc(subject)+'}',r'\newcommand{\docrev}{2.1}',r'\newcommand{\docdate}{21 September 2026}',r'\input{preamble_spec}',r'\begin{document}',r'\begin{titlepage}\vspace*{24mm}',r'{\Huge\bfseries\color{accent}'+esc(title)+r'\par}\vspace{12mm}',r'{\Large '+esc(subject)+r'\par}\vfill',r'\identblock{'+kind+r'}\end{titlepage}',r'\tableofcontents']
...
def ports(mod):
    ...
    return result
def modulebody(mod):
    # Template 6.x: the purpose leads, then the interface, then how the module works.
    return (esc(FUNCTIONS[mod])+'\n'+r'\subsubsection*{Interface description}'+'\n'
            +paramtable(mod)+porttable(mod)+r'\subsubsection*{Functional description}'+'\n')
def paramtable(mod):
...
def cpu_design():
    ...
    d.page('Scope and original requirements',T(['Source','Role'],[('RV32I 3-Stage Pipeline CPU.pdf','Original internship brief, locally stored in cpu/docs/brief/.'),('cpu/hdl/rv32i_cpu_top.v','Implemented top-level contract documented here.')],[.43,.57])+T(['Topic','Initial requirement','Implemented contract'],CPU_REQ,[.14,.37,.49]))
    ...
    d.page('Module parameters',paramtable('rv32i_cpu_top')+T(['Width class','Contract'],[('Architectural widths','Instructions, addresses and data are fixed at 32 bits; GPR indices are 5 bits.'),('Interrupt widths','16 mask/pending bits, 4-bit source ID. These are fixed widths, not module parameters.'),('Prediction capacity','BP_ENTRIES and RAS_DEPTH change internal capacity, not external bus widths.')],[.26,.74]))
...
def pic_design():
    ...
    d.page('Scope and original requirements',T(['Source','Role'],[('Programmable Interrupt Controller with Advanced Scheduling','Original internship brief in cpu/docs/brief/.'),('cpu/hdl/pic.v and axi_lite_slave.v','Implemented interface and register-access behaviour.')],[.48,.52])+T(['Topic','Initial requirement','Implemented contract'],PIC_REQ,[.15,.35,.50]))
    ...
    d.page('Module configuration and integration constraints',paramtable('pic')+T(['Fixed quantity','Value'],[('Source slots','16; each merges a hardware line and software request.'),('Priority bands','4; configurable urgency, not additional modules.'),('Physical nesting depth','16; run-time NEST_MAX defaults to 8.'),('AXI width','32-bit address/data; four strobes; three protection bits; two response bits.')],[.35,.65])+T(['Constraint','Required integration behaviour'],[('Address window','Decode exactly a 256-byte window. Only address bits [7:2] are used; upper bits alias and low bits select the containing word.'),('IRQ input synchrony','irq_src_i must be synchronous to clk_i; synchronize asynchronous peripheral sources outside the PIC.'),('CPU handshake','Claim refers to the offer sampled one cycle earlier. Never assert claim and EOI together.'),('Depth limit','A lower NEST_MAX does not discard existing active contexts; it suppresses new offers until sufficient EOIs occur.'),('One CPU','One offer / mask / claim / EOI interface per PIC instance.'),('Protection','AWPROT/ARPROT accepted and ignored; no access-control mechanism in this block.'),('Implementation target','Behavioural RTL simulation. No synthesis / frequency claim.')],[.26,.74]))
    ...
    for name,(offset,rows) in PIC_FIELDS.items():
        d.page('Register fields: '+name,fields(name,offset,rows))
    ...
    d.page('Deadline escalation',wave('pic_escalation','Reordered bands, source 2 deadline = 10 cycles: escalation precedes the registered offer changing from source 1 to 2.')+T(['Setting / event','Behaviour'],[('DEADLINE=0','Timer disabled and held at zero.'),('Nonzero deadline','Count while enabled pending and not active. CPU masking does not stop the pending timer.'),('MODE=0','Jump to TARGET when deadline expires.'),('MODE=1','Move to the next strictly greater urgency from BAND_CONFIG, not simply band number minus one.'),('MULTI=1','Repeat at each deadline. In bump mode, the effective band stays unchanged if no higher urgency exists.'),('Claim / no longer pending','Clear elapsed wait and escalation state; configured priority applies again.'),('Status','SRCx_STATUS.ESC is wait-local; INT_STATUS.ESC is sticky until RW1C.')],[.29,.71]))
...
('CPU-02','rv32i_tb_isa','1287-instruction independently generated reference trace; nominal, delayed and stalled memories.'),
...
('CPU-05','rv32i_tb_traps','All implemented synchronous causes; precedence, wrong-path errors and precise architectural state.'),
...
('PIC-02','pic_tb_reference','Independent priority model: all 24 urgency permutations, request/enable/CPU masks and ties.'),
...
('PIC-08','soc_tb_pic_sources / nest / escalate','Interrupt source routing, nesting and escalation in the integrated SoC.')]
...
def verification(kind):
    ...
    d.page('Verification objective and evidence',T(['Item','Contract'],[('Requirement source','RV32I 3-Stage Pipeline CPU.pdf' if cpu else 'Programmable Interrupt Controller with Advanced Scheduling (original PIC brief).'),('Device under test','rv32i_cpu_top and its RTL modules.' if cpu else 'pic and axi_lite_slave.'),('Functional verdict','Every expected value and response must match; errors=0 and test_done=1 are required.'),('Protocol verdict','VALID/payload stability, response ordering and outstanding transaction limits; clocked at posedge.'),('Failure handling','Mismatch, timeout, incomplete test, assertion failure or coverage-gate failure returns a failing run.'),('Limits of evidence','Directed simulation and reference-model checks, not a formal proof or a synthesis/timing result.')],[.29,.71])+T(['Simulator / flow','Recorded evidence (18–21 September 2026)'],[('ModelSim','22 CPU/PIC configurations; zero mismatches.'),('Verilator --assert','16 CPU/PIC benches and the CPU functional-coverage gate.'),('Integrated SoC','25 ModelSim configurations and 16 assertion-enabled benches.'),('PULP comparison','Decoder sequence plus 96 register-slave strobe/order/stall cases and RO/unmapped/reset cases.')],[.29,.71]))
    ...
    d.page('Assertions, monitors and bind',T(['File / mechanism','What it checks'],[('axi_lite_monitor.v','Passive ModelSim protocol checks and transaction counts; valid traffic must occur.'),('axi_lite_sva.sv','Channel stability, legal response codes, request/response ordering, one outstanding transaction per direction.'),('rv32i_cpu_core_sva.sv' if cpu else 'pic_sva.sv','Pipeline/trap/forwarding invariants.' if cpu else 'Offer eligibility, CPU mask, priority threshold, depth transitions, mutual exclusion and empty EOI.'),('rv32i_bind_core_sva.sv','bind attaches checker instances to RTL module types. The checkers read DUT signals; they drive nothing and add no port.'),('rv32i_bind_sva.sv','Attaches the nominal CPU functional-coverage model; coverage is separate from the shared assertion bind.'),('run_verilator.sh','Compiles assertions with --assert and fails the run on an assertion failure. ModelSim ASE runs the procedural monitors instead; the SVA execute under Verilator.')],[.36,.64])+T(['Non-vacuous check','Evidence'],[('Valid traffic','Read/write counts and exercised tests prevent a disconnected monitor from being counted as coverage.'),('Preconditions','RO-write test first creates nonzero state; async-reset test first creates pending/nonzero state.'),('Negative scenarios','Watch an interval for forbidden offers/claims; do not infer absence from one sample.')],[.32,.68]))
    d.page('Environment setup and named scripts',r'\needspace{7\baselineskip}\subsubsection{Structure}'+'\n'+T(['Folder','Contents'],[
        ...
        +r'\needspace{7\baselineskip}\subsubsection{Scripts}'+'\n'+T(['Step','Working directory','Command / script','Result'],[('1','Repository root','make asm','Regenerate programs and independent reference vectors.'),('2','cpu/debug/sim','vsim -c -do "do regress.do; quit -f"','Compile and run 22 CPU/PIC configurations.'),('3','cpu/debug/sim (Bash / WSL)','bash run_verilator.sh','Lint, functional coverage and 16 benches with SVA.'),('4','soc/debug/sim','vsim -c -do "do regress.do; quit -f"','25 integration configurations.'),('5','Repository root (Bash / WSL)','bash soc/debug/sim/run_verilator.sh','SoC lint and 16 assertion-enabled benches.'),('6','soc/debug/sim (Bash / WSL)','bash run_pulp_compare.sh','Pinned external decoder/register comparison.'),('7','cpu/debug/sim','vsim -c -do "do waves.do"','Run 8 checked benches and export VCD traces.'),('8','Repository root','python cpu/docs/tools/waveforms.py','SVG/PDF/PNG figures and source-window manifest.')],[.06,.22,.43,.29])+r'\needspace{7\baselineskip}\subsubsection{Setup and configuration}'+'\n'+T(['Tool / setup','Requirement'],[('ModelSim','vsim/vlog on PATH; run from the named simulation directory so file lists resolve.'),('Verilator','Verilator 5.050 used for the recorded run; C++ build tools, Bash and Python 3 are required.'),('PULP','Git and network only for the initial checkout; explicit revision pins recorded in the runner.'),('Wave rendering','Python with matplotlib; raw VCD files produced by waves.do.'),('Configuration','Latency/stall overrides are read back after elaboration; seeds are fixed for repeatability.')],[.26,.74]))
