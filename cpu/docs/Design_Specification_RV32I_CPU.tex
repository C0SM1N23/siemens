...
Verification Specification — CPU & Rev. 2.2, 23 September 2026 & C.-A. Bunea & Companion document. \\ \addlinespace[3pt]
Design Specification — PIC & Rev. 2.2, 23 September 2026 & C.-A. Bunea & Other side of the CPU–PIC interrupt link. \\ \addlinespace[3pt]
...
\noindent The table sets each requirement of the brief RV32I 3-Stage Pipeline CPU against the contract implemented in rv32i\_cpu\_top, including the points where the two differ. In the SoC, the instruction port reaches the instruction memory, the data port reaches data memory, the SRAM and the peripheral registers through the SoC decoder, and the interrupt link connects directly to the PIC; the address map is in soc/README.md.\par\medskip
{\small
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth,height=132mm,keepaspectratio]{figures/cpu_essential.pdf}\caption{Three stages with pipeline boundaries, data paths and essential control feedback; widths are in bits. PIC and memories are external.}\end{figure}
...
Redirect priority & Synchronous trap / accepted interrupt, then MRET, then branch correction; redirect requires S2 to advance. \\*
\bottomrule\end{longtable}}
...
Unsupported encoding & Illegal-instruction exception, including M/A/C instructions and FENCE.I. \\*
\bottomrule\end{longtable}}
...
Call / return & RAS\_DEPTH=8 by default; zero disables the stack. JAL/JALR hints use x1/x5. Return entries use the stack top, falling back to the BTB target when empty. Full pushes replace the oldest entry; empty pops have no effect. \\*
\bottomrule\end{longtable}}
...
\noindent The two memory ports and the PIC link are independent interfaces. All ports are digital and synchronous to the rising edge of clk\_i; rst\_n\_i is the only asynchronous input.\par\medskip
{\small
...
RAS\_DEPTH & 8 & Return-address stack entries; zero disables it. \\*
\bottomrule\end{longtable}}
\noindent Instructions, addresses and data are 32 bits and GPR indices 5 bits. The interrupt link has 16 mask and pending bits and a 4-bit source ID. BP\_ENTRIES and RAS\_DEPTH set internal capacity only; no port width depends on a parameter.\par\medskip
{\small
...
\texttt{cpu\_in\_trap\_o} & output & 1 & CPU has an active trap context. \\*
\bottomrule\end{longtable}}
...
Integration & Distribute reset to CPU and bus responders. Release it with the system reset controller; release between clock edges is also supported. \\*
\bottomrule\end{longtable}}
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth]{waves/cpu_reset.pdf}\caption{Instruction request interrupted by asynchronous reset at 27 ns.}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth]{waves/cpu_fetch.pdf}\caption{Instruction fetch after rst\_n release at 123 ns.}\end{figure}
{\small
\begin{longtable}{@{}L{0.2500\dimexpr\linewidth-12pt\relax}L{0.7500\dimexpr\linewidth-12pt\relax}@{}}
\toprule
...
Redirect & An accepted request cannot be cancelled. Retain its route / drain the response before using the new path. \\*
\bottomrule\end{longtable}}
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth]{waves/cpu_load.pdf}\caption{Load: lsu\_active holds S2 until the data response completes.}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth]{waves/cpu_store.pdf}\caption{Store: address and write data are accepted independently; B completes the store.}\end{figure}
{\small
...
Protection & AWPROT/ARPROT=011: data, non-secure, privileged. \\*
\bottomrule\end{longtable}}
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth]{waves/cpu_irq.pdf}\caption{Interrupt claim: cpu\_irq\_ack\_o is a full-cycle registered pulse.}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=\linewidth]{waves/cpu_redirect.pdf}\caption{Branch correction: redirect invalidates younger work.}\end{figure}
{\small
\begin{longtable}{@{}L{0.2400\dimexpr\linewidth-12pt\relax}L{0.7600\dimexpr\linewidth-12pt\relax}@{}}
\toprule
...
Nested software & Save mepc, mstatus and handler context before enabling nested interrupts; restore before MRET. \\*
\bottomrule\end{longtable}}
...
0xF14 & mhartid & RO & HART\_ID & Instance identifier. \\*
\bottomrule\end{longtable}}
{\small
...
Unimplemented inhibit & 0x320 (mcountinhibit) is absent and traps. \\*
\bottomrule\end{longtable}}
...
[3] & MIE & RW + HW & 0 & Global enable; trap clears it; MRET restores MPIE. \\*
\bottomrule\end{longtable}}
...
[15:0] & Reserved & RAZ/WI & 0 & Reads zero; writes ignored. \\*
\bottomrule\end{longtable}}
...
[1:0] & MODE & RW / WARL & 0 & 0 direct; 1 vectored. Values 2/3 read back as direct. \\*
\bottomrule\end{longtable}}
{\small
...
External interrupt, vectored & BASE + 4 × (16 + source ID). \\*
\bottomrule\end{longtable}}
...
[1:0] & Alignment & fixed & 0 & Reads zero; writes discarded. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & VALUE & RW & 0 & Software-owned scratch word. \\*
\bottomrule\end{longtable}}
...
[4:0] & CODE & RW + HW & 0 & Hardware cause code; external source n is 16+n. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & VALUE & RW + HW & 0 & Faulting address or illegal instruction; zero where the cause supplies no payload. \\*
\bottomrule\end{longtable}}
{\small
...
[8] & I & fixed WARL & 1 & RV32I base integer set. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & VALUE & RO & 0 & No implementation-specific identifier assigned. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & VALUE & RO & HART\_ID & Instance parameter; use unique values for multiple harts. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & LOW / HIGH & RW & 0 & Each address accesses one half of a 64-bit counter. Explicit writes suppress that cycle’s event increment and preserve the other half. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & EVENT & fixed WARL & 1..5 & Fixed event IDs; writes accepted and ignored. \\*
\bottomrule\end{longtable}}
...
\textbf{Bits} & \textbf{Field} & \textbf{Access type} & \textbf{Reset} & \textbf{Description} \\ \midrule
\endhead
[31:0] & VALUE & fixed WARL & 0 & Reads zero; writes accepted and ignored. \\*
\bottomrule\end{longtable}}
{\small
...
mhpmcounter7 & WFI sleep cycle. \\*
\bottomrule\end{longtable}}
\noindent A 64-bit counter is read high, low, high; the read is repeated when the two high values differ.\par\medskip
...
6 & For nesting, save trap CSRs/context before re-enabling MIE. & Inner handler cannot overwrite the outer return state. \\*
\bottomrule\end{longtable}}
{\small
\begin{longtable}{@{}L{0.3400\dimexpr\linewidth-12pt\relax}L{0.6600\dimexpr\linewidth-12pt\relax}@{}}
\toprule
...
Behavioural verification target & No area, frequency, power or physical timing result is specified. \\*
\bottomrule\end{longtable}}

\clearpage
\section{Reuse and reusability}
\noindent The core contains no third-party component. rv32i\_alu, rv32i\_imm\_gen, rv32i\_decode and rv32i\_regfile can be reused unchanged in another RV32I datapath; rv32i\_branch\_predictor is sized by ENTRIES and RAS\_DEPTH. Reusing the whole core:\par\medskip
{\small
\begin{longtable}{@{}L{0.2300\dimexpr\linewidth-12pt\relax}L{0.7700\dimexpr\linewidth-12pt\relax}@{}}
\toprule
...
Verification & Run regress.do and run\_verilator.sh, then test the integration map and slave latency of the target system. \\*
\bottomrule\end{longtable}}
...
\textbf{Parameter} & \textbf{Default} & \textbf{Meaning / valid setting} \\ \midrule
\endhead
RESET\_PC & 32'h0000\_0000 & Word-aligned first fetch address. \\*
\bottomrule\end{longtable}}
{\small
\begin{longtable}{@{}L{0.3400\dimexpr\linewidth-36pt\relax}L{0.1000\dimexpr\linewidth-36pt\relax}L{0.1000\dimexpr\linewidth-36pt\relax}L{0.4600\dimexpr\linewidth-36pt\relax}@{}}
\toprule
...
\texttt{fetch\_fault\_o} & output & 1 & Instruction response reported an AXI error. \\*
\bottomrule\end{longtable}}
...
\texttt{update\_link\_i} & input & 32 & Return address PC+4. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
\noindent The lookup is combinational and an update is committed on posedge. The prediction and update policies, including aliases, cold entries and trap preservation, are specified in section 3.3.\par\medskip
...
\texttt{funct7\_o} & output & 7 & Instruction function field, bits [31:25]. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{illegal\_o} & output & 1 & Encoding is not supported by the implemented instruction set. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\endhead
\texttt{instr\_i} & input & 32 & 32-bit instruction word to decode or check. \\ \addlinespace[3pt]
\texttt{imm\_out\_o} & output & 32 & Immediate expanded to 32 bits for the decoded instruction format. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{rs2\_data\_o} & output & 32 & Second register operand. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{funct3\_i} & input & 3 & Instruction function field; selects operation or memory size/sign. \\ \addlinespace[3pt]
\texttt{funct7\_i} & input & 7 & Instruction function field; distinguishes arithmetic and shift variants. \\ \addlinespace[3pt]
\texttt{result\_o} & output & 32 & 32-bit ALU result for the selected operation. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{pc\_src\_o} & output & 1 & Next PC takes branch\_target\_o instead of the sequential address. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{dbus\_rready\_o} & output & 1 & Receiver can accept the read response. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{mem\_misaligned\_o} & output & 1 & Address is not naturally aligned; the AXI issue is blocked. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\textbf{Parameter} & \textbf{Default} & \textbf{Meaning / valid setting} \\ \midrule
\endhead
HART\_ID & 32'd0 & Value returned by mhartid. \\*
\bottomrule\end{longtable}}
{\small
\begin{longtable}{@{}L{0.3400\dimexpr\linewidth-36pt\relax}L{0.1000\dimexpr\linewidth-36pt\relax}L{0.1000\dimexpr\linewidth-36pt\relax}L{0.4600\dimexpr\linewidth-36pt\relax}@{}}
\toprule
...
\texttt{ev\_wfi\_sleep\_i} & input & 1 & WFI sleep cycle. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{fwd\_rs2\_o} & output & 1 & Use S3 writeback data as the second S2 register operand. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\texttt{wb\_data\_o} & output & 32 & Selected 32-bit value to write to the destination GPR. \\*
\bottomrule\end{longtable}}
\subsubsection{Functional description}
...
\textbf{Limitation} & \textbf{Workaround} \\ \midrule
\endhead
FENCE.I is not implemented and traps as an illegal instruction. & Load instruction memory before the core executes from it; code cannot modify itself. \\ \addlinespace[3pt]
Misaligned loads and stores trap (cause 4 / 6). & Align the data, or emulate the access in the trap handler. \\ \addlinespace[3pt]
The user counters cycle, time and instret (0xC00–0xC02) and mcountinhibit (0x320) are absent and trap. & Read mcycle and minstret in machine mode. \\ \addlinespace[3pt]
The trap-kind stack holds 16 levels, interrupts and exceptions combined. & Keep the combined nesting depth at or below 16, for example through NEST\_MAX. \\*
\bottomrule\end{longtable}}
...
CAUSE\_ECALL\_M & 5'd11 & Trap cause \\*
\bottomrule\end{longtable}}
