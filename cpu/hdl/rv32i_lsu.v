// S2 load/store unit: one AXI4-Lite transaction at a time.
// AW and W handshakes are tracked separately. Completion requires the response.
// The caller holds funct3 through completion and rejects misalignment before start.
...
module rv32i_lsu (
    ...
    always @(*) begin
        case (funct3_i[1:0])
            ...
            2'b00:   st_lanes = {4{st_data_i[7:0]}};
            2'b01:   st_lanes = {2{st_data_i[15:0]}};
            default: st_lanes = st_data_i;
        endcase
    end
    ...
    always @(*) begin
        case (funct3_i[1:0])
            ...
            2'b00:   st_strb = 4'b0001 << addr_i[1:0];
            2'b01:   st_strb = 4'b0011 << addr_i[1:0];
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) state_q <= S_IDLE;
        else if (start) state_q <= we_i ? S_WR : S_RD;
        else if (done_o) state_q <= S_IDLE;
    end
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) ar_sent_q <= 1'b0;
        else if (start) ar_sent_q <= dbus_arvalid_o && dbus_arready_i;
        else if (done_o) ar_sent_q <= 1'b0;
        else if (dbus_arvalid_o && dbus_arready_i) ar_sent_q <= 1'b1;
    end

    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) aw_sent_q <= 1'b0;
        else if (start) aw_sent_q <= dbus_awvalid_o && dbus_awready_i;
        else if (done_o) aw_sent_q <= 1'b0;
        else if (dbus_awvalid_o && dbus_awready_i) aw_sent_q <= 1'b1;
    end

    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) w_sent_q <= 1'b0;
        else if (start) w_sent_q <= dbus_wvalid_o && dbus_wready_i;
        else if (done_o) w_sent_q <= 1'b0;
        else if (dbus_wvalid_o && dbus_wready_i) w_sent_q <= 1'b1;
    end

    // command latch (reset, see header)
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) addr_q <= 32'd0;
        else if (start) addr_q <= addr_i;
    end

    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) wdata_q <= 32'd0;
        else if (start) wdata_q <= st_lanes;
    end

    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) wstrb_q <= 4'd0;
        else if (start) wstrb_q <= st_strb;
    end
