// AXI4-Lite register interface for PIC and timer.
// AW and W are buffered independently. One outstanding request per direction.
// Reads sample rd_data/rd_ok at AR acceptance; writes pulse wr_en after AW and W.
...
module axi_lite_slave (
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        ...
        else if (w_hs) wdata_q <= s_axi_wdata_i;
    end

    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) wstrb_q <= 4'd0;
        else if (w_hs) wstrb_q <= s_axi_wstrb_i;
    end
