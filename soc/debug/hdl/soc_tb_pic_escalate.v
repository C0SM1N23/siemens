    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      entries=%0d, claims seen=%0d", dut.dmem_inst.mem[SB_ENTRIES], claim_n
                );
        end else begin
            if ($test$plusargs("verbose"))
                $display("   program finished after %0d cycles\n", timeout);

            $display("-- Escalation while pending --");
            ...
            $display("\n-- Handler order after escalation --");
