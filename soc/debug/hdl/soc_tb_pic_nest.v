    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      entries=%0d depth now=%0d max depth=%0d claims=%0d eois=%0d",
            ...
            $display("-- Outer interrupt handler --");
            check(32'h0000_0109, dut.dmem_inst.mem[SB_VEC_OUTER],
                  "entered for the less urgent source");
            check(32'd1, dut.dmem_inst.mem[SB_DEPTH_OUT], "one level open while it runs");
            ...
            check(32'd2, max_depth[31:0], "the depth reached two and never went past it");
            ...
            check(32'd0, depth_jump[31:0], "the depth only ever moved one level at a time");

            $display("\n-- Claim and EOI counts --");
