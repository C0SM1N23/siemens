...
module soc_tb_dma_irq;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (dma_int_enable_nz) unmasked_q <= 1'b1;
        end
    end
    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      CH0_STATUS=%0d irq_masked=%0d irq_after=%0d dma_irq=%b",
                    dut.dmem_inst.mem[SB_CH0_STATE],
                    dut.dmem_inst.mem[SB_IRQ_MASKED],
                    dut.dmem_inst.mem[SB_IRQ_AFTER],
                    dma_irq
                );
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      cycles with a request while masked: DMA %0d, controller %0d",
                    irq_while_masked,
                    cpu_irq_while_masked
                );
            ...
            check(32'd0, dut.dmem_inst.mem[SB_MIP], "the core saw nothing pending either");
            ...
            $display("\n-- Pending interrupt after unmask --");
