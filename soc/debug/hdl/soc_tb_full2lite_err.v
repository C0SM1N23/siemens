...
module soc_tb_full2lite_err;
    ...
    reg  [15:0] response_pattern;
    reg         pattern_enable;
    reg  [ 1:0] pattern_bresp;
    assign l_bresp = pattern_enable ? pattern_bresp : slave_bresp;
    ...
    // Scripted responses are independent of the slave's register decode.
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) pattern_bresp <= RESP_OKAY;
        else if (s_wr_en) pattern_bresp <= response_pattern[wbeat_cnt*2+:2];
    end
    ...
    initial begin
        ...
        check({30'd0, RESP_SLVERR}, {30'd0, beat_rresp[3]}, "beat 3 reports SLVERR");
        bad = 0;
        for (i = 0; i < 8; i = i + 1) if (i != 3 && beat_rresp[i] !== RESP_OKAY) bad = bad + 1;
        ...
        arm_write_err(NO_ERR, NO_ERR);
        do_write(32'h100, 8'd7);
        check(32'd3, {30'b0, last_bresp}, "DECERR then SLVERR remains DECERR");
        response_pattern = 16'h000E;  // beat 0 SLVERR, beat 1 DECERR, then OKAY
        arm_write_err(NO_ERR, NO_ERR);
    ...
    initial begin
        #200000;
        $fatal(1, "FAIL: tb_full2lite_err timeout");
    end
