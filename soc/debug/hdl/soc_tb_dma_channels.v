...
module soc_tb_dma_channels;
    ...
    always @(posedge clk or negedge rst_n) begin : update_grants_of
        integer i;
        if (~rst_n) begin
            for (i = 0; i < 4; i = i + 1) grants_of[i] <= 0;
        end else begin
            for (i = 0; i < 4; i = i + 1) if (gnt_pulse[i]) grants_of[i] <= grants_of[i] + 1;
        end
    end
    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            for (i = 0; i < 4; i = i + 1)
            $display(
                "      channel %0d: state=%0d, granted %0d times",
                i,
                dut.dmem_inst.mem[SB_ST0+i],
                grants_of[i]
            );
        end else begin
            ...
            $display("-- Channel completion --");
            for (i = 0; i < 4; i = i + 1)
            check(ST_DONE, dut.dmem_inst.mem[SB_ST0+i], "this channel reported STATE_DONE");
            ...
            for (i = 0; i < 4; i = i + 1)
            check(32'd0, dut.dmem_inst.mem[SB_MISM0+i], "no mismatched word for this channel");
            ...
            $display("\n-- Arbiter grant activity --");
            for (i = 0; i < 4; i = i + 1) begin
                if (grants_of[i] > 0) begin
                    if ($test$plusargs("verbose"))
                        $display(
                            "PASS: channel %0d was granted the bus %0d times", i, grants_of[i]
                        );
                end else begin
