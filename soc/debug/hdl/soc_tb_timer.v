    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      timer line high for %0d cycles, %0d claims on source 7",
                    tmr_high_cycles,
                    claims_of_7
                );
            if (claims_of_7 > 1)
                if ($test$plusargs("verbose"))
                    $display(
                        "      -> the handler is being re-entered: the level was not released"
                    );
        end else begin
            if ($test$plusargs("verbose"))
                $display("   program finished after %0d cycles\n", timeout);

            $display("-- Single timer interrupt --");
            ...
            $display("\n-- Compare threshold timing --");
            if (dut.dmem_inst.mem[SB_MTIME] >= dut.dmem_inst.mem[SB_CMP]) begin
                if ($test$plusargs("verbose"))
                    $display(
                        "PASS: the time at the trap (%0d) had reached the compare value (%0d)",
                        dut.dmem_inst.mem[SB_MTIME],
                        dut.dmem_inst.mem[SB_CMP]
                    );
            end else begin
            ...
            $display("\n-- Interrupt clear after compare update --");
