...
module soc_tb_stress #(
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (dut.arb_dmem.req[0] && dut.arb_dmem.req[1]) arb_both_req <= arb_both_req + 1;
        end
    end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (dut.dbus_rvalid && dut.dbus_rresp == 2'b11) decerr_seen <= decerr_seen + 1;
            if (dut.dbus_bvalid && dut.dbus_bresp == 2'b11) decerr_seen <= decerr_seen + 1;
        end
    end
    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if (dut.sram_inst.mem_array_inst.mem[i] !== 32'h5A5A0000 + i) begin
                errors = errors + 1;
                $display("FAIL: SRAM word %0d = 0x%08h, expected 0x%08h", i,
                         dut.sram_inst.mem_array_inst.mem[i], 32'h5A5A0000 + i);
            end
            begin
                if ($test$plusargs("verbose"))
            ...
            if (!COVERAGE_GATE || dut.dmem_inst.mem[SB_COLL_IRQ] > 0) begin
                if ($test$plusargs("verbose"))
                    $display(
                        "PASS: the SRAM collision interrupt reached the CPU (%0d times)",
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "     DMEM grants  CPU / DMA        : %0d / %0d", arb_gnt_cpu, arb_gnt_dma
                );
            if ($test$plusargs("verbose"))
            ...
            if (!COVERAGE_GATE) $display("   (coverage floors not gated in this configuration)");

            if (!COVERAGE_GATE || arb_both_req >= MIN_CONTENDED) begin
                if ($test$plusargs("verbose"))
                    $display(
                        "PASS: the DMEM arbiter was contended for %0d cycles (floor %0d)",
                        arb_both_req,
                        MIN_CONTENDED
                    );
            end else begin
            ...
            if (!COVERAGE_GATE || (arb_gnt_cpu > 0 && arb_gnt_dma > 0)) begin
                if ($test$plusargs("verbose"))
            ...
            if (!COVERAGE_GATE || (sram_both_ports > 0)) begin
                if ($test$plusargs("verbose"))
            ...
            if (!COVERAGE_GATE || (sram_conflicts > 0)) begin
                if ($test$plusargs("verbose"))
                    $display("PASS: real address conflicts occurred and were resolved");
            end else begin
                errors = errors + 1;
                $display(
                    "FAIL: no address conflict ever occurred - the collision logic is untested");
            end

            if (!COVERAGE_GATE || (decerr_seen > 0)) begin
                if ($test$plusargs("verbose"))
