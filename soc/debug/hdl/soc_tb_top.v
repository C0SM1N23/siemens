    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      CH0_STATUS=%0d  irq_count=%0d  dma_irq=%b  cpu_irq=%b",
                    dut.dmem_inst.mem[SB_STATUS],
                    dut.dmem_inst.mem[SB_IRQ_CNT],
                    dma_irq,
                    cpu_irq
                );
        end else begin
            ...
            check(1'b0, {31'd0, cpu_irq}, "the interrupt line is released after the handler");
            check(4'b0000, {28'd0, dma_irq},
                  "the DMA dropped its request once the channel was cleared");
            check(1'b0, {31'd0, sram_irq}, "no SRAM collision was raised by a clean transfer");
        end
