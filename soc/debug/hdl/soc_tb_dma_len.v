    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            for (s = 0; s < NSLOT; s = s + 1) begin
                ...
                if ($test$plusargs("verbose"))
                    $display(
                        "   %0d bytes (%0d words): %0d word(s) wrong inside, %0d past the end",
                        len_bytes[s],
                        words,
                        short_ct,
                        over
                ...
                if (short_ct != 0) begin
                    errors = errors + 1;
                    $display("FAIL: %0d-byte transfer did not deliver %0d word(s)", len_bytes[s],
                             short_ct);
                end
                if (over != 0) begin
                    errors = errors + 1;
                    $display(
                        "FAIL: %0d-byte transfer wrote %0d word(s) past the end, from byte %0d",
