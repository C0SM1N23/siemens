...
module soc_tb_dma_err;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (dut.bridge_inst.s_bvalid_o && dut.bridge_inst.s_bready_i) begin
                if (dut.bridge_inst.s_bresp_o !== RESP_OKAY) full_err_bursts <= full_err_bursts + 1;
            end
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            if (dut.bridge_inst.s_bvalid_o && dut.bridge_inst.s_bready_i) begin
                if (dut.bridge_inst.s_bresp_o !== RESP_OKAY) begin
                end else full_ok_bursts <= full_ok_bursts + 1;
            end
        end
    end
    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      clean run state=%0d, overrun state=%0d",
                    dut.dmem_inst.mem[SB_ST_GOOD],
                    dut.dmem_inst.mem[SB_ST_BAD]
                );
            if ($test$plusargs("verbose"))
                $display(
                    "      error beats on the fabric=%0d, failed bursts=%0d",
                    lite_err_beats,
                    full_err_bursts
                );
        end else begin
            ...
            $display("-- In-range transfer --");
            check(ST_DONE, dut.dmem_inst.mem[SB_ST_GOOD], "the clean transfer completed");
            check(32'd0, dut.dmem_inst.mem[SB_MISM], "and every word of it matched");

            $display("\n-- Out-of-range bus response --");
            if (lite_err_beats > 0) begin
                if ($test$plusargs("verbose"))
                    $display(
                        "PASS: %0d beats were answered with an error response", lite_err_beats
                    );
            end else begin
            ...
            check(ST_ERROR, dut.dmem_inst.mem[SB_ST_BAD], "the channel ended in its error state");

            $display("\n-- In-range beat data --");
            check(32'hBAD00000, dut.dmem_inst.mem[SB_FIRST],
                  "the first in-window word was written");
            check(32'hBAD00003, dut.dmem_inst.mem[SB_LAST], "the last in-window word was written");
