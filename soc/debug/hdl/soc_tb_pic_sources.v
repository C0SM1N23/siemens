...
module soc_tb_pic_sources;
    ...
    always @(posedge clk or negedge rst_n) begin : update_claims_of
        integer i;
        if (~rst_n) begin
    ...
    initial begin
        ...
        if (dut.dmem_inst.mem[SB_DONE] !== DONE_MARKER) begin
            ...
            if ($test$plusargs("verbose"))
                $display(
                    "      handler entries so far = %0d, claims = %0d",
                    dut.dmem_inst.mem[SB_ENTRIES],
                    total_claims
                );
            for (i = 0; i < 16; i = i + 1)
            if (claims_of[i] == 0) $display("      slot %0d was never claimed", i);
        end else begin
            if ($test$plusargs("verbose"))
                $display("   program finished after %0d cycles\n", timeout);

            $display("-- Handler cause for all 16 sources --");
            for (i = 0; i < 16; i = i + 1)
            check(32'h0000_0100 | i, dut.dmem_inst.mem[SB_VEC0+i],
                  "the source number the handler read back");

            $display("\n-- One claim per source --");
            for (i = 0; i < 16; i = i + 1) check(32'd1, claims_of[i][31:0], "claims on this slot");

            $display("");
            check(32'd16, dut.dmem_inst.mem[SB_ENTRIES], "sixteen handler entries");
            check(32'd16, total_claims[31:0], "sixteen claims in total");
            check(32'd0, dut.dmem_inst.mem[SB_DEPTH], "every level was released again");
