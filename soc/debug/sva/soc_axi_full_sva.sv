...
module soc_axi_full_sva #(
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) begin
            ...
            if (ar_hs) begin
                ...
                if (rlast_i) begin
                end else rd_beat_q <= rd_beat_q + 8'd1;
            end
        end
    end
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) begin
            ...
            if (w_hs && !wlast_i) wr_beat_q <= wr_beat_q + 8'd1;
        end
    end
    ...
    always @(posedge clk_i or negedge rst_n_i) begin
        if (~rst_n_i) begin
            ...
            if (w_hs && wlast_i) wr_data_done_q <= 1'b1;
            if (b_hs) begin
                wr_data_done_q <= 1'b0;
            end
        end
    ...
    ar_payload_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) arvalid_i && !arready_i |=> $stable(
        araddr_i
    ) && $stable(
        arlen_i
    ) && $stable(
    ...
    ar_one_outstanding :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) ar_hs |-> !rd_out_q)
    else $error("[%0s] second read burst issued while one is in flight", NAME);

    r_needs_outstanding :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) rvalid_i |-> rd_out_q)
    else $error("[%0s] RVALID with no outstanding AR", NAME);

    r_valid_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) rvalid_i && !rready_i |=> rvalid_i)
    else $error("[%0s] RVALID dropped before RREADY", NAME);

    r_payload_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) rvalid_i && !rready_i |=> $stable(
        rdata_i
    ) && $stable(
        rresp_i
    ) && $stable(
    ...
    r_last_on_final_beat :
    assert property (@(posedge clk_i) disable iff (!rst_n_i)
    r_hs && rlast_i |-> rd_beat_q == rd_len_q)
    else $error("[%0s] RLAST on beat %0d of a burst of length %0d", NAME, rd_beat_q, rd_len_q);

    r_no_last_before_final :
    assert property (@(posedge clk_i) disable iff (!rst_n_i)
    r_hs && rd_beat_q != rd_len_q |-> !rlast_i)
    else $error("[%0s] RLAST asserted early", NAME);

    r_beat_in_range :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) rd_out_q |-> rd_beat_q <= rd_len_q)
    ...
    aw_payload_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) awvalid_i && !awready_i |=> $stable(
        awaddr_i
    ) && $stable(
        awlen_i
    ) && $stable(
    ...
    aw_one_outstanding :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) aw_hs |-> !wr_out_q)
    else $error("[%0s] second write burst issued while one is in flight", NAME);

    w_valid_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) wvalid_i && !wready_i |=> wvalid_i)
    else $error("[%0s] WVALID dropped before WREADY", NAME);

    w_payload_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) wvalid_i && !wready_i |=> $stable(
        wdata_i
    ) && $stable(
        wstrb_i
    ) && $stable(
    ...
    w_needs_address :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) w_hs |-> wr_out_q)
    else $error("[%0s] W beat with no outstanding AW", NAME);

    w_last_on_final_beat :
    assert property (@(posedge clk_i) disable iff (!rst_n_i)
    w_hs && wlast_i |-> wr_beat_q == wr_len_q)
    else $error("[%0s] WLAST on beat %0d of a burst of length %0d", NAME, wr_beat_q, wr_len_q);

    w_no_last_before_final :
    assert property (@(posedge clk_i) disable iff (!rst_n_i)
    w_hs && wr_beat_q != wr_len_q |-> !wlast_i)
    else $error("[%0s] WLAST asserted early", NAME);

    w_no_beat_after_last :
    assert property (@(posedge clk_i) disable iff (!rst_n_i) w_hs |-> !wr_data_done_q)
    ...
    b_valid_stable :
    assert property (@(posedge clk_i) disable iff (!rst_n_i)
    bvalid_i && !bready_i |=> bvalid_i && $stable(
        bresp_i
    ))
    ...
    generate
        ...
        ar_hs |-> (arburst_i == BURST_INCR || arburst_i == BURST_FIXED)
                ...
                $error(
                    "[%0s] read burst outside the supported subset: burst=%0b size=%0b",
                    NAME,
                    arburst_i,
                    arsize_i
                );
        ...
        aw_hs |-> (awburst_i == BURST_INCR || awburst_i == BURST_FIXED)
                ...
                $error(
                    "[%0s] write burst outside the supported subset: burst=%0b size=%0b",
                    NAME,
                    awburst_i,
                    awsize_i
                );
    ...
    cov_write_burst_8 :
    cover property (@(posedge clk_i) disable iff (!rst_n_i) aw_hs && awlen_i == 8'd7);
    cov_r_backpressure :
    cover property (@(posedge clk_i) disable iff (!rst_n_i) rvalid_i && !rready_i);
    cov_w_backpressure :
    cover property (@(posedge clk_i) disable iff (!rst_n_i) wvalid_i && !wready_i);
    cov_b_slverr :
    cover property (@(posedge clk_i) disable iff (!rst_n_i) b_hs && bresp_i == 2'b10);
    cov_r_slverr :
    cover property (@(posedge clk_i) disable iff (!rst_n_i) r_hs && rresp_i == 2'b10);
