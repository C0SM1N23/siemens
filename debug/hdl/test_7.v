    ...
    initial begin
        ...
        fork
            ...
            begin
                ...
                // =========T07: Abort / Resume Test========
                $display("\n--- [T07] RUNNING: Suspend (Abort) and Resume ---");
                setup_dma_transfer(32'h0300, 32'h5000, 32'h6000, 32'h80, 32'hC3C3C3C3); // 128 bytes
                start_channel(ADDR_CH1_DESC_ADDR, 32'h0300, 32'h00200008); 
                
                #200; // Let one burst pass through
                $display("          Sending ABORT command...");
                axi_lite_write(ADDR_CH1_CONTROL, 32'h00000002, 0); // Bit 1 = Abort
                
                #300; // Wait for suspension
                $display("          Sending RESUME command...");
                axi_lite_write(ADDR_CH1_CONTROL, 32'h00000005, 0); // Bit 2 = Resume, Bit 0 = Enable
                
                wait_channel_done(ADDR_CH1_STATUS);
                $display(">>> [T07] DONE (Check Waveforms to visually verify suspension)");
