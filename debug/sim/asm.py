...
def write_if_changed(path, text):
    """Rewrite only when the content differs; returns True if written."""
    try:
        with open(path) as f:
            if f.read() == text:
                return False
...
def main():
    ...
    top = max(mem) + 1
    hex_text = "".join(f"{mem.get(i, 0):08x}\n" for i in range(top))

    # `<stem>_sym.vh` next to the hex: one `define ADDR_<label> per symbol,
    ...
    sym_text = "// Generated by asm.py: label addresses for the testbench.\n" \
    ...
    # keep the GUI's ".s newer than .hex" auto-assemble check settled.
    if not write_if_changed(out_path, hex_text):
        os.utime(out_path)
    if not write_if_changed(sym_path, sym_text):
        print(f"{sym_path}: unchanged, mtime kept (no Verilator rebuild)")
