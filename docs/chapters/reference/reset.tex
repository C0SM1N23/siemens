...
The narrower rule --- reset only what correctness requires --- would be
defensible. Two findings in this design argue against it.

\paragraph{An unreset flop can reach a bus output.} In the shared AXI4-Lite
slave, \sig{BRESP} and \sig{RRESP} are combinational functions of a stored error
flag and are driven \emph{whether or not} the corresponding \sig{VALID} is
asserted. With those flags unreset, both peripherals presented X on their
response codes from time zero until their first transaction completed. This was
present in the delivered design, and is the finding that motivated the change.

\paragraph{Any register may become architecturally visible.} The interrupt
...
Behind both is one point. ``The valid bit makes the X harmless'' is a claim about
every consumer of every unreset bit, and it has to stay true after the next
change; it also holds only in a four-state simulation, since a two-state model or
a synthesised netlist resolves the X silently and the two then disagree.
Resetting the flop retires the claim.
...
figure is quoted. Removing it again is confined to the predictor's
payload registers in \file{branch\_predictor.v}, and leaves the design
functionally identical --- the predictor is never believed before its valid bit
is set.
...
slave holding \sig{READY} high in reset would have accepted a phantom
address.

\section{How the rule is verified}

Most of the policy is measured rather than asserted, and the part that is not
is marked as such below.

Four benches carry the evidence, and each checks the same three properties for
its block: that every register reads back the value listed above on its first
access after release; that nothing in the block holds X after release, including
state with no register view, probed directly through hierarchical references;
and that a reset asserted \emph{off a clock edge} from a fully configured,
actively used state returns every one of those registers to the same value ---
which is what distinguishes a real asynchronous reset from a simulation initial
value.

\begin{table}[H]
\centering
\caption{Which reset values are demonstrated, and by what}
\label{tab:resetevidence}
\begin{tabular}{@{}L{0.32\linewidth}L{0.28\linewidth}L{0.28\linewidth}@{}}
\toprule
\textbf{Table} & \textbf{Demonstrated by} & \textbf{Coverage} \\
\midrule
Interrupt controller, Table~\ref{tab:picreset} &
  \file{tb\_pic\_reset}, \S\ref{sec:vp-picreset} &
  All 56 mapped words, twice; the nesting stack probed directly \\
Machine timer and shared slave, Table~\ref{tab:tmrreset} &
  \file{tb\_mtimer\_regs}, \S\ref{sec:vp-mtimer} &
  All four registers; the slave's response codes and \sig{RDATA} sampled during
  reset \\
CSR file, Table~\ref{tab:csrreset} &
  \file{tb\_csr\_ro}, \S\ref{sec:vp-csr} &
  Every implemented CSR, before and after a second reset \\
Branch predictor rows of Table~\ref{tab:cpureset} &
  \file{tb\_bp}, \S\ref{sec:vp-bp} &
  All five arrays probed directly; all 128 index positions; the RAS pointer,
  counter and entries \\
\midrule
Pipeline registers, fetch stage and LSU command latch, remaining rows of
  Table~\ref{tab:cpureset} &
  \textbf{Read from the source; not observed by a directed check} &
  Covered indirectly: the X-hygiene assertions would fire on an X reaching a bus
  output, and every run starts from reset \\
\bottomrule
\end{tabular}
\end{table}

The last row is the honest limit of the evidence. Those flops have no register
interface, and no bench probes them; what the regression does show is that
nothing derived from them reaches a bus output as X, on every run of every
configuration. A bench that probes them directly, in the style of
\file{tb\_bp}'s first section, would close it.

\paragraph{The check has teeth.} Run against the previous revision of the RTL,
the reset bench fails on precisely the flops that revision left unreset --- the
two AXI response-code flags and the interrupt controller's nesting-stack arrays.
That is the evidence that it detects the defect it was written for, rather than
passing because nothing is being looked at.
