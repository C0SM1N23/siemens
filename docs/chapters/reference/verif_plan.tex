...
\begin{enumerate}[label=2.\arabic*,leftmargin=12mm,itemsep=1pt]
  ...
  \item Read all 16 \reg{SRCx\_CONFIG}, all 16 \reg{SRCx\_SW\_TRIG}, all 16
        ...
        map --- and compare each against the reset column of
        Table~\ref{tab:picmap}.
\end{enumerate}
...
Table~\ref{tab:csrmap}. \reg{mstatus} is the one that is not zero, because MPP
...
\paragraph{Why it was added.} Chapter~\ref{ch:fetch} documents the RAS
overflow, underflow and replace-top behaviours and the \sig{RAS\_DEPTH}~=~0
tie-off. The directed program never nests calls deeper than three, so none of
the four was exercised anywhere in the regression, and no run looked at the
predictor's state out of reset. The behaviour was implemented and documented but
not validated, which is a different thing from being wrong.
