...
riscv-arch-test suite or randomised instruction stream has been run. Functional
coverage stands at 88 of 92 bins with the four misses explained in
Chapter~\ref{ch:verif}. The testbench interconnect covers two slaves; richer
fabrics, including a fabric that generates its own decode errors, arrive with the
real interconnect.

\section{Simulation artefacts}

Pipeline and command-latch payload registers are intentionally not reset
(decision D1). Several idle-bus payload signals therefore read \texttt{X} in
waveforms until their first transaction. This is legal AXI --- the payload must be
stable only while \sig{VALID} is asserted --- and the protocol checkers pass, but
it is worth knowing before a waveform review. \sig{s\_axi\_rdata} in the shared
slave was given a reset value of zero for exactly this reason.
