...
\begin{rtlnote}
This wire exists because the first version of the block used the CPU's single
\sig{cpu\_in\_trap} level to release the serviced source. That works for one
level and cannot express \emph{which} nesting level has finished. Adding a
distinct pop event was the change that made real nesting possible;
\sig{cpu\_in\_trap} is still driven, but the PIC does not use it.
\end{rtlnote}
...
After the interrupt is acknowledged and the source returns to idle the escalated
priority is dropped: the effective band reverts to the configured band and the
counter resets, so escalation never accumulates across events. Each request
starts from the priority software actually programmed.
\reg{SRCx\_STATUS.ESC} clears with it, while the global \reg{INT\_STATUS.ESC}
stays sticky so the event itself is not lost.

\begin{rtlnote}
The effective-band update orders the escalation branch \emph{before} the
track-configuration branch, so on the cycle a deadline is missed the escalation
wins --- \sig{escalated[s]} only rises on the following cycle.
\end{rtlnote}
