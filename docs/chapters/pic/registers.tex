...
{[2]}     & \sig{ESC}       & 0 & Missed its deadline and was escalated. Clears at the claim, together with \sig{EFF\_BAND} and \sig{DDL\_TIMER}; the sticky \reg{INT\_STATUS.ESC} does not. \\
{[3]}     & \sig{SPUR}      & 0 & The current claim was spurious. Clears on the return; the sticky \reg{SPURIOUS\_LOG} bit does not. \\
{[5:4]}   & \sig{EFF\_BAND} & 00 & The band actually in use now; differs from \sig{BAND} between an escalation and the claim that ends it. \\
