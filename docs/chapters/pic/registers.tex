...
\midrule
{[0]}       & \sig{PEND}      & The source has an enabled request. Does not imply it is the one being offered. \\
{[1]}     & \sig{ACTIVE}    & In service, on the nesting stack. Cannot be offered again until its end-of-interrupt. \\
{[2]}     & \sig{ESC}       & Missed its deadline and was escalated. Clears when the source returns to idle. \\
{[3]}     & \sig{SPUR}      & The current claim was spurious. \\
{[5:4]}   & \sig{EFF\_BAND} & The band actually in use now; differs from \sig{BAND} after an escalation. \\
{[31:16]} & \sig{DDL\_TIMER}& Cycles elapsed on the deadline counter. Runs while pending and unserviced. \\
\bottomrule
\end{tabular}
\end{table}
