...
\begin{deviation}
\textbf{Integration rule, not a defect.} A source enabled in \reg{INT\_ENABLE}
but masked in \reg{mie} stops at step 7, \emph{after} winning step 4. It
therefore remains the controller's standing offer and starves every less urgent
source. The two masks are independent by design, so the discipline is: a source
that software does not want should be disabled in \reg{INT\_ENABLE}, not merely
masked in \reg{mie}. The test program respects this and the effect is verified
in the channel sweep.
\end{deviation}
...
valid whenever \sig{depth} is non-zero. Entries at or above \sig{depth} are never
read, so only the pointer needs a reset for correctness. All three are reset all
the same, because \reg{NEST\_STATUS.TOP\_ID} and \reg{ACTIVE\_VEC.ID} read
\sig{stack\_id} out to software (Chapter~\ref{ch:reset}); the entries are probed
directly in the reset bench, Section~\ref{sec:vp-picreset}.
