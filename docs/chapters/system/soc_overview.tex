...
\midrule
\sig{cpu\_irq}      & PIC $\to$ CPU & Level request. High while the PIC is offering a source. \\
\sig{cpu\_irq\_vec[3:0]} & PIC $\to$ CPU & Identifier of the offered source; becomes trap cause $16+v$. \\
\sig{cpu\_irq\_ack} & CPU $\to$ PIC & One-cycle claim pulse, asserted when the CPU commits to the handler. \\
\sig{cpu\_irq\_eoi} & CPU $\to$ PIC & One-cycle pulse on an interrupt-returning \sig{MRET}; pops one nesting level. \\
\sig{cpu\_in\_trap} & CPU $\to$ observer & High from trap entry until \sig{MRET}. Not used by the PIC; kept for observability and assertions. \\
\bottomrule
\end{tabular}
\end{table}
