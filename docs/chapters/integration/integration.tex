...
\begin{center}
\ttfamily\small
wr\_commit = aw\_got\_q \&\& w\_got\_q \&\& !s\_axi\_bvalid;
\end{center}
...
\begin{center}
\ttfamily\small
s\_axi\_awready = !aw\_got\_q \&\& !s\_axi\_bvalid;\\
s\_axi\_wready  = !w\_got\_q  \&\& !s\_axi\_bvalid;
\end{center}
...
\begin{tikzpicture}[>={Stealth[length=2.4mm]},semithick]
  ...
  \node[st,above right=13mm and 30mm of idle] (gotaw) {\scriptsize GOT\_AW};
  \node[st,below right=13mm and 30mm of idle] (gotw)  {\scriptsize GOT\_W};
  \node[st,right=30mm of idle,xshift=30mm] (resp) {\scriptsize RESP};

  \draw[->,draw=accent] (idle) to[bend left=10] node[lbl,above left=0mm and -2mm]{AW hs} (gotaw);
  \draw[->,draw=accent] (idle) to[bend right=10] node[lbl,below left=0mm and -2mm]{W hs} (gotw);
  \draw[->,draw=accent] (gotaw) to[bend left=10] node[lbl,above right=0mm and -2mm]{W hs} (resp);
  \draw[->,draw=accent] (gotw) to[bend right=10] node[lbl,below right=0mm and -2mm]{AW hs} (resp);
  \draw[->,draw=accent] (idle) to node[lbl,above,align=center]{AW + W\\\scriptsize same cycle} (resp);
  \draw[->,draw=accent] (resp) to[bend left=52]
      node[lbl,below,align=center,pos=0.5]{BREADY --- BVALID drops, flags clear} (idle);
  \node[align=left,font=\scriptsize,below=15mm of idle,text width=64mm]
    {\textbf{Reset:} both flags clear, \texttt{BVALID} low.\\
     \texttt{wr\_en} pulses for one cycle on entry to RESP.\\
     \texttt{BRESP} = \texttt{wr\_ok} ? OKAY : SLVERR, captured at commit.};
\end{tikzpicture}
\caption{Write handshake of \file{axi\_lite\_slave.v}, encoded in
\sig{aw\_got\_q}, \sig{w\_got\_q} and \sig{s\_axi\_bvalid} rather than in a state
register.}
\label{fig:slavewr}
\end{figure}
...
Every flop in this module is reset, and two of them are the reason the whole
delivery adopted that rule. \sig{BRESP} and \sig{RRESP} are combinational
functions of \sig{bresp\_err\_q} and \sig{rresp\_err\_q}, and they are driven
\emph{whether or not} the corresponding \sig{VALID} is asserted. An earlier
revision left both flags unreset on the argument that they are only meaningful
while \sig{VALID} is high --- true of the protocol, and irrelevant to what the
pins actually carry, which was \texttt{X} from time zero until the first
transaction of each direction completed. \sig{s\_axi\_rdata\_o} is reset for the
same reason. Chapter~\ref{ch:reset} states the resulting policy; the reset bench
checks all three signals while reset is still asserted
...
The failure mode of that arrangement was found during verification and is stated
in Chapter~\ref{ch:soc}: a source enabled in the PIC but masked in \reg{mie} is
never claimed, so it remains the PIC's offer forever and starves everything less
urgent. This is not an RTL defect --- both masks behave as specified --- but it is
an integration rule the system must respect.
