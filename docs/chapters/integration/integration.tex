...
\sig{s\_axi\_rdata} resets to zero, so the port reads as zero rather than
\texttt{X} before its first transaction. The two response-error flags
(\sig{rresp\_err\_q}, \sig{bresp\_err\_q}) are not reset; they are only meaningful
while the corresponding \sig{VALID} is high, which is low out of reset.
...
(\file{debug/hdl/axi\_lite\_dec2.v}). The first splits the \texttt{0x3xxx\_xxxx}
window from everything else; the second splits that window into the PIC and the
machine timer. Anything outside a mapped region returns DECERR from the memory
model, which the CPU turns into a precise access fault.
