...
The interrupt is a registered level: \sig{irq $\leftarrow$ (mtime\_q >=
...
\begin{center}
\ttfamily\small
mtime\_q[31:0] <= wr\_time\_lo ? lane\_merge(mtime\_inc[31:0], wdata, wstrb)\\
\hspace*{6.2em}: mtime\_inc[31:0];
\end{center}
...
ones, so writing the low half first is safe. \sig{irq} follows the comparison one
