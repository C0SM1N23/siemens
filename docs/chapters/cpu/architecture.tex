...
\begin{tikzpicture}[x=1mm,y=1mm,font=\small]
  % Routing rules for this figure: no line crosses a box or a label, and every
  % arrow ends on a block or on a junction rail. The two return paths along the
  % bottom cross each other once, at (58,-64) in open space - the predictor
  % update is tapped right of the forwarding return but lands left of it, so
  % one crossing is unavoidable.
  ...
  % --- pipeline registers ------------------------------------------------
  % Topped at y = 2 so the redirect path can run above them at y = 5 without
  % passing through either bar.
  ...
  % --- result rail: the four candidates the mux selects between ----------
  % Each result leaves DX/WB and meets the others on the rail, so every one of
  % them ends somewhere instead of stopping at the register edge.
  ...
  \node[lbl,above=0.5mm] at (30,-64) {update at commit};

  % --- redirect: out of branch_unit, up the gap between the operand rail
  %     and the alu column, then above both pipeline registers ------------
  \draw[flowd] (bu.north) -- (112,-14) -- (96,-14) -- (96,5) -- (3,5) -- (fu.north);
  % clear of the "S2 decode + execute" heading, which is centred at x = 77
  \node[lbl,above=0.5mm] at (34,5) {redirect};

  % --- writeback value back into the forwarding mux ----------------------
  % Along the bottom. The direct path would run at y = -31, straight through
  % the DX/WB register and across the operand rail.
  \draw[flow] (wb.south) -- (161,-68) -- (58,-68) -- (58,-24) -- (fwd.west);
  \node[lbl,above=0.5mm] at (100,-68) {S3$\to$S2 forwarding};
\end{tikzpicture}}

\caption{CPU block structure. Each box is a module in \file{hdl/}, except the
forwarding mux, which is inline logic in \file{cpu\_top.v}.}
\label{fig:cpu}
\end{figure}
