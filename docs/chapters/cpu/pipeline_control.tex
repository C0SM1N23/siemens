...
\sig{valid} bit that has a reset, and a payload that does not.

Nothing downstream looks at the payload unless \sig{valid} is set, so a bubble is
simply \sig{valid}~=~0 and a flush or a reset produces a guaranteed no-operation
whatever the payload happens to hold. The alternative considered was injecting an
\sig{addi x0,x0,0} encoding, which would have required the flush path to drive 32
correct bits and every downstream decoder to interpret them --- more hardware and
more ways to be wrong.

Two consequences follow directly. The reset tree stays small, since only the
valid bits are in it. And the payload flops do not toggle on bubbles, which saves
switching power.

\begin{deviation}
\textbf{Accepted side effect.} Because payload registers are not reset, several
signals derived from them read \texttt{X} in simulation until they are first
written --- notably the \sig{dbus} address, write data and strobe outputs, which
come from the LSU command latch, and \sig{RRESP[1]}/\sig{BRESP[1]} on the
peripheral ports. This is legal AXI: the protocol requires the payload to be
stable and meaningful only while the corresponding \sig{VALID} is asserted, and
\sig{VALID} is low throughout. The protocol monitors and the X-hygiene assertions
pass. It remains visual noise when reviewing waveforms.
\end{deviation}
