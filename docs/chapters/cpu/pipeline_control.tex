...
The payload is nonetheless reset (Chapter~\ref{ch:reset}). That is a second,
independent decision, and the two are worth keeping apart: \sig{valid} makes the
payload \emph{irrelevant} downstream, while the reset makes it \emph{defined} on
the nets that read it combinationally. Three of those nets decide the point:
\sig{ifdx\_instr\_q} feeds the decoder, whose \sig{MemRead}, \sig{MemWrite} and
\sig{illegal} outputs reach the AXI \sig{VALID} signals; \sig{ifdx\_pc\_q} feeds
the PC+4 adder, the branch-target adder and the predictor update port;
\sig{dxwb\_result\_q} feeds the writeback multiplexer and the S3$\to$S2
forwarding path. The reset values are chosen so that the idle datapath is not
merely defined but harmless --- the canonical NOP \texttt{0x0000\_0013} for the
instruction word, \sig{RESET\_PC} for program addresses, zero elsewhere.

\begin{rtlnote}
This was changed in revision 2.0 of the delivery. The earlier revision reset only
the valid bits, on the argument that a valid bit makes the payload
unobservable. That argument holds inside the pipeline and fails at the block
boundary: the same reasoning had been applied to the two AXI response-code flags
in the shared slave, where it was wrong --- \sig{BRESP} and \sig{RRESP} are driven
whether or not the corresponding \sig{VALID} is high, so both peripherals
presented \texttt{X} on the bus from time zero. Rather than audit the argument
flop by flop, the rule was made absolute. The cost is stated in
...
\ttfamily\small
s2\_advance   = \textasciitilde lsu\_busy \& \textasciitilde wfi\_wait;\\
redirect      = (trap\_take | mret\_exec | mispredict) \& s2\_advance;\\
if\_dx\_we     = s2\_advance;\\
if\_dx\_bubble = redirect | \textasciitilde fetch\_valid;\\
dx\_squash     = trap\_take;
\end{minipage}
\end{center}
