...
The rule is applied across the whole delivery rather than only where a net is
read combinationally, because ``the valid bit makes the X harmless'' is a claim
about every consumer of every unreset bit. Applied to the shared slave's
response-code flags, that same reasoning put \texttt{X} on a bus output
(Chapter~\ref{ch:reset}). The cost is stated in Section~\ref{sec:resetcost}.
