...
\label{tab:causes}
\begin{tabular}{@{}clL{0.42\linewidth}l@{}}
\toprule
\textbf{Code} & \textbf{Cause} & \textbf{Condition} & \textbf{\reg{mtval}} \\
\midrule
0  & Instruction address misaligned & Jump target not 4-byte aligned. & target \\
1  & Instruction access fault & Fetch returned SLVERR/DECERR and reached S2. & fetch PC \\
2  & Illegal instruction & Unknown encoding, or disallowed CSR access. & instruction \\
3  & Breakpoint & \sig{EBREAK}. & PC \\
4  & Load address misaligned & Checked before the access; no transaction issued. & address \\
5  & Load access fault & Read response SLVERR/DECERR. & address \\
6  & Store address misaligned & As cause 4. & address \\
7  & Store access fault & Write response SLVERR/DECERR. & address \\
11 & Environment call from M-mode & \sig{ECALL}. & 0 \\
16--31 & External interrupt, source 0--15 & \reg{mcause}[31] set; source $v$ gives cause $16+v$. & 0 \\
\bottomrule
\end{tabular}
\end{table}

The chain is evaluated in exactly the order listed, and it selects the cause and
the \reg{mtval} payload together, so the cause list exists in one place in the
source.
